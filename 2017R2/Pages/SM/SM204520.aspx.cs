...
public partial class Pages_SM_SM204520 :PXPage
{
	...
	protected override void OnLoad(EventArgs e)
	{
		...
		foreach (string id in buttons)
		{
			...
			if (btn != null)
			{
				btn.ImageSet = Sprite.AliasTree;
				//btn.ImageKey = "Component";
				btn.ImageAlign = GeneralAlign.Right;
				btn.CssClass = "Button toolBox";
				btn.AlignLeft = true;
				btn.Attributes["draggable"] = "true";
				btn.TextAlign = HorizontalAlign.Left;
				btn.RenderAsButton = false;
	...
	protected void TreePageControls_NodeMove(object sender, PXTreeMoveEventArgs e)
	{
		...
		LayoutEditor.MoveControl(node[0], before==null?null : before[0]);
	}
	...
	protected void TreePageControls_DataBound(object sender, EventArgs e)
	{
		...
		var tree = sender as PXTreeView;
		if (tree != null)
		{
			tree.ToolBar.CallbackUpdatable = true;
			var btn = (PXToolBarButton)tree.ToolBar.Items["Filter"];
			btn.Pushed = tree.RootNode != null && tree.RootNode.ChildNodes.Count == 1;
		}
	}
