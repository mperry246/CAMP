    ...
    */
    ...
    public static Object GetStaffAppointments(DateTime scheduledDate, int? node, int branchID, int? branchLocationID, bool unassignedAppointmentFlag)
    {
        ...
        if (node == 0)
        {
            ...
            for (int i = 0; i < total && (start + i < results.Count); i++)
            {
                PXResult<EPEmployee, FSAppointmentEmployee, FSAppointment, FSServiceOrder, BAccount> result = (PXResult<EPEmployee, FSAppointmentEmployee, FSAppointment, FSServiceOrder, BAccount>)results[start + i];
