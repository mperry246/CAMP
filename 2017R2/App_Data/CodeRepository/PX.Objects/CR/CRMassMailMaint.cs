	...
	public class CRMassMailMaint : PXGraph<CRMassMailMaint, CRMassMail>
	{
		...
		public virtual IEnumerable previewMail(PXAdapter a)
		{
					...
					}) == WebDialogResult.OK)
			{
				...
				SendMassMail(accountId, mailTo, null, null, mails, false, this, MassMails.Current);
				Save.Press();
			}
		...
		private static void SendMassMail(int? accountId, string mailTo, string mailCc, string mailBcc, IEnumerable<Recipient> recievers, bool linkToEntity, CRMassMailMaint graph, CRMassMail current)
		{
				...
				case CRMassMailSourcesAttribute.Campaign:
                    ...
                    foreach (CRCampaign list in Campaigns.Select())
						if (list.Selected == true)
						{
                            ...
                            if (list.SendFilter == SendFilterSourcesAttribute._ALL)
                            {
                            ...
                            {                                
                                ...
                                Where<Contact.noMassMail, NotEqual<True>,
                                    And<Contact.noEMail, NotEqual<True>,
                                    And<CRCampaignMembers.campaignID, Equal<Required<CRCampaignMembers.campaignID>>,
                                    And<CRCampaignMembers.emailSendCount, Equal<Zero>>>>>>
                                .Select(this, list.CampaignID))
                                {
                                    var contact = (Contact)item;                                    
                                    var address = contact.EMail;
