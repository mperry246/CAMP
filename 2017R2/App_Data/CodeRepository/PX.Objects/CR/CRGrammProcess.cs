...
namespace PX.Objects.CR
{
	...
	public class CRGrammProcess : PXGraph<CRGrammProcess>
	{
		...
		private class Processor
		{
			...
			private static readonly string[] ContactTypes = {ValidationTypesAttribute.LeadAccount,ValidationTypesAttribute.LeadContact};
			private static readonly string[] AccountTypes = {ValidationTypesAttribute.LeadAccount, ValidationTypesAttribute.Account };
			...
			public bool PersistGrams(Contact contact, bool requireRecreate = false)
			{
				try
				{
					...
					if (graph.Caches[typeof (Contact)].GetStatus(contact) == PXEntryStatus.Deleted)
					{
			...
			private class ValiadtionDefinition
			{
			...
			private ValiadtionDefinition Definition
			{
				get
				{
				...
				foreach (string validationType in types)
				{
					...
					foreach (CRValidationRules rule in Definition.TypeRules[type])
					{
						...
						else
						{
							...
							CRGrams gram = new CRGrams();
							gram.FieldValue = value.ToString();
