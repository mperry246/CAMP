...
namespace PX.Objects.CR
{
	...
	public class CRCommunicationInbox : CRCommunicationBase<Where<CRSMEmail.isIncome, Equal<True>, 
                                    ...
                                    And<CRSMEmail.mpstatus, NotEqual<MailStatusListAttribute.archived>>>>>
	{
		...
		public virtual IEnumerable relate(PXAdapter adapter)
		{
			...
			if (lint)
			{
				...
				foreach (PXResult<CRSMEmail, EMailAccount, EPView> email in SelectedList())
				{
                    ...
                    Type entityType = PXBuildManager.GetType(entityFilter.Current.Type, false);
				    object row = helper.GetEntityRow(entityType, entityFilter.Current.RefNoteID);
				    Type graphType = helper.GetPrimaryGraphType(row, false);
				    PXGraph graph = PXGraph.CreateInstance(graphType);
				    graph.Caches[row.GetType()].Current = row;
		...
		public virtual IEnumerable create(PXAdapter adapter)
		{
			...
			PXAdapter a = new PXAdapter(new SOOrderEntry.DummyView(graph, new Select<CRSMEmail>(), SelectedList().Cast<object>().ToList()))
			{
				MassProcess = true,
				Menu = adapter.Menu
			};
