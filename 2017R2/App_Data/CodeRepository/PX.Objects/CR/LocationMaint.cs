...
namespace PX.Objects.CR
{
	...
	public partial class LocationARAccountSub : IBqlTable
	{
		...
		[Account(BqlField = typeof(CRLocation.cARAccountID), DisplayName = "AR Account", DescriptionField = typeof(Account.description), Required = true)]
		...
		where Where : class, IBqlWhere, new()
	{
		...
		protected virtual void EstablishCTaxZoneRule(Action<Type, bool> setCheck)
		{
			...
			bool isUsageTypeRequired = customerClass != null && customerClass.RequireAvalaraCustomerUsageType == true && PXAccess.FeatureInstalled<FeaturesSet.avalaraTax>();
			setCheck(typeof(Location.cAvalaraCustomerUsageType), isUsageTypeRequired);
		}
	...
	public class LocationValidator
	{
		public virtual bool ValidateCustomerLocation(PXCache cache, BAccount baccount, ILocation location)
		{
			...
			res &= validationHelper.SetErrorEmptyIfNull<Location.cSalesSubID>(location.CSalesSubID);
			...
			if (location.IsARAccountSameAsMain != true)
			{
				...
				res &= validationHelper.SetErrorEmptyIfNull<Location.cARSubID>(location.CARSubID);
			}
		...
		public virtual bool ValidateVendorLocation(PXCache cache, Vendor vendor, ILocation location)
		{
			...
			if (vendor.TaxAgency == true)
			{
				...
				res &= validationHelper.SetErrorEmptyIfNull<Location.vExpenseSubID>(location.VExpenseSubID);
			}
			...
			if (location.IsAPAccountSameAsMain != true)
			{
				...
				res &= validationHelper.SetErrorEmptyIfNull<Location.vAPSubID>(location.VAPSubID);
			}
