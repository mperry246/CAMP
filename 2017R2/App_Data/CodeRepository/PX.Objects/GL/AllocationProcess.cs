	...
	public class AllocationProcess : PXGraph<AllocationProcess>
	{
		...
		{			
			foreach (GLAllocation it in PXSelectReadonly<GLAllocation,
													...
													And<GLAllocation.gLAllocationID,NotEqual<Required<GLAllocation.gLAllocationID>>>>>>.Select(this,aAlloc.SortOrder, aAlloc.GLAllocationID))
			{
				GLAllocation alloc= it;                
				Batch batch = PXSelectReadonly2<Batch, InnerJoin<GLAllocationHistory, On<GLAllocationHistory.batchNbr, Equal<Batch.batchNbr>,
														And<GLAllocationHistory.module, Equal<Batch.module>>>>,
														Where<GLAllocationHistory.gLAllocationID, Equal<Required<GLAllocationHistory.gLAllocationID>>,
														And<Batch.tranPeriodID, Equal<Required<Batch.tranPeriodID>>>>>.SelectWindowed(this,0,1,alloc.GLAllocationID,aFiscalPeriod);
				if (batch != null && batch.BatchNbr != null)
				{
					continue; //Already processed
				}
				if (isApplicable(alloc, aFiscalPeriod)) return true; //Check, if allocation is applicable for the period				
			}
			return false;
		}
		...
		static int ComparePeriods(string aPeriod1, string aPeriod2, bool aIgnoreYear) 
		{
			int year1, nbr1;
			int year2, nbr2;
			FiscalPeriodUtils.TryParse(aPeriod1, out year1, out nbr1);
			FiscalPeriodUtils.TryParse(aPeriod2, out year2, out nbr2);
			if (!aIgnoreYear) 
			{
