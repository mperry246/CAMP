	...
	public class GLConsolSetupMaint : PXGraph<GLConsolSetupMaint>
	{
		...
		public IEnumerable branches()
		{
			var branches = PXSelectJoin<Branch,
										...
										Where<Ledger.postInterCompany, Equal<True>,
											And2<Where<Ledger.defBranchID, IsNull, 
														Or<Branch.branchID, NotEqual<Ledger.defBranchID>>>,
											And<Where<Ledger.consolAllowed, Equal<True>,
			...
			using (PXSoapScope scope = new PXSoapScope(consolSetup.Url, consolSetup.Login, consolSetup.Password))
			{
				...
				foreach (Ledger ledger in foreign.Ledgers.Select())
				{
					...
					if (l != null)
					{
						...
						if (l.Description != ledger.Descr || l.PostInterCompany != ledger.PostInterCompany || l.BalanceType != ledger.BalanceType)
						{
							l.Description = ledger.Descr;
							l.PostInterCompany = ledger.PostInterCompany;
		...
		protected virtual void GLSetup_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			{
				if (ConsolSetupRecords.Cache.GetStatus(detail) == PXEntryStatus.Notchanged)
				{
					ConsolSetupRecords.Cache.SetStatus(detail, PXEntryStatus.Updated);
				}
			}
		}
		...
		protected virtual void GLConsolSetup_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<GLConsolSetup.sourceBranchCD>(sender, e.Row, coledger != null && (coledger.PostInterCompany == true || coledger.BalanceType == LedgerBalanceType.Report));
		}
