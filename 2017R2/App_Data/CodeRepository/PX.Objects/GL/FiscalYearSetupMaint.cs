...
namespace PX.Objects.GL
{
	public class FiscalYearSetupMaint : YearSetupGraph<FiscalYearSetupMaint, FinYearSetup, FinPeriodSetup>
	{
		...
		{
			PXSelectBase select = new PXSelect<FinYear>(this);
		...
		{
			return (int)PXSelectGroupBy<FinYearSetup, Aggregate<Count>>.Select(this).RowCount > 0;
		}
		...
		protected override bool CheckForPartiallyDefinedYear()
		{
			...
			FinYear unclosedYear = null;
			foreach (FinYear fy in PXSelect<FinYear>.Select(this))
			{
				int count = 0;
	...
	public abstract class YearSetupGraphBase<TGraph, TYearSetup, TPeriodSetup, TPeriodSetupSelect> : PXGraph<TGraph>
		where TGraph : PXGraph
		where TYearSetup : class, IYearSetup, IBqlTable, new()
		where TPeriodSetup : class, IPeriodSetup, IBqlTable, new()
		where TPeriodSetupSelect : PXSelectBase<TPeriodSetup>
	{
		public abstract class YearSetup
		{
			...
			public abstract class begFinYear : IBqlField { }
			public abstract class finPeriods : IBqlField { }
			public abstract class userDefined : IBqlField { }
			public abstract class adjustToPeriodStart : IBqlField { }
			public abstract class periodLength : IBqlField { }
			public abstract class periodsStartDate : IBqlField { }
			public abstract class periodType : IBqlField { }
			public abstract class hasAdjustmentPeriod : IBqlField { }
			public abstract class endYearCalcMethod : IBqlField { }
			public abstract class endYearDayOfWeek : IBqlField { }
			public abstract class yearLastDayOfWeek : IBqlField { }
		}
		...
		protected virtual IEnumerable DecrFirstYear(PXAdapter adapter)
		{
			...
			{
				List<TYearSetup> result = new List<TYearSetup>(1);
				foreach (TYearSetup year in adapter.Get<TYearSetup>())
				{

					if (year != null && year.BegFinYear.HasValue)
					{
						this.DoIncrementFirstYear(year, true);
						result.Add(year);
					}
		...
		//public PXAction<TYearSetup> incrementFirstYear;
		//[PXUIField(DisplayName = "Increment First Year", Visible = true)]
		//[PXButton(ImageKey = PX.Web.UI.Sprite.Main.AddNew)]
		//protected virtual IEnumerable IncrementFirstYear(PXAdapter adapter)
		//{
		...
		//        if (year != null && year.BegFinYear.HasValue)
		//        {
		//            this.DoIncrementFirstYear(year, false);
		//            result.Add(year);
		//        }
		//    }
		//    return result;
		...
		[PXButton(Tooltip = Messages.AutoFillFiscalYearSetupToolTip)]
		...
		{
			TYearSetup year = (TYearSetup)this.FiscalYearSetup.Current;
			//if (this.hasUnfinishedPeriods)
			//{
			//    throw new PXException(Messages.FiscalYearsHaveUnclosedPeriods);
			//}
		...
		protected virtual void TYearSetupOnRowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			//bool isWeekBasedPeriod = fes.FPType == FiscalPeriodSetupCreator.FPType.FourFourFive || fes.FPType == FiscalPeriodSetupCreator.FPType.FourFiveFour || fes.FPType == FiscalPeriodSetupCreator.FPType.FiveFourFour || fes.FPType == FiscalPeriodSetupCreator.FPType.Week;

			bool isCustomPeriods = (fes.FPType == FiscalPeriodSetupCreator.FPType.Custom);

			if (isFixedLengthPeriod == false)
			{
				bool adjustToPeriodStart = false;
				if (fes.PeriodsStartDate.HasValue && fes.BegFinYear.HasValue && fes.BegFinYear != fes.PeriodsStartDate)
				{
					adjustToPeriodStart = true;
				}
				fes.AdjustToPeriodStart = adjustToPeriodStart;
			}
			...
			PXUIFieldAttribute.SetEnabled<YearSetup.finPeriods>(cache, fes, !isFixedLengthPeriod && allowAnyModification && (isCustomPeriods && !this.HasPartiallyDefinedYear));
			//PXUIFieldAttribute.SetEnabled<YearSetup.userDefined>(cache, fes, !isFixedLengthPeriod && (!this.hasUnfinishedPeriods));
			PXUIFieldAttribute.SetEnabled<YearSetup.userDefined>(cache, fes, false);
			PXUIFieldAttribute.SetEnabled<PeriodSetup.endDateUI>(this.Periods.Cache, null, !isFixedLengthPeriod && (isCustomPeriods));
			this.decrFirstYear.SetEnabled(!allowChangeStartingDate && allowAnyModification);
			...
			PXUIFieldAttribute.SetEnabled<YearSetup.adjustToPeriodStart>(cache, fes, allowChangeStartingDate && allowAnyModification);
		...
		private void DoIncrementFirstYear(TYearSetup year, bool negative)
		{
			int increment = negative ? -1 : 1;
			DateTime? newStartDate = year.BegFinYear.Value.AddYears(increment);
			FiscalPeriodSetupCreator<TPeriodSetup> creator = new FiscalPeriodSetupCreator<TPeriodSetup>(year.FirstFinYear,
																			year.PeriodsStartDate.Value,
																			year.FPType,
																			year.HasAdjustmentPeriod.Value,
																			year.AdjustToPeriodStart.Value,
																			year.PeriodLength,
																			year.BegFinYear.Value,
																			year.EndYearCalcMethod,
																			year.EndYearDayOfWeek.Value
																			);
			try
			{
				this.skipPeriodReset = true;
				this.FiscalYearSetup.Cache.SetValueExt<YearSetup.begFinYear>(year, newStartDate);
			}
			finally
			{
				this.skipPeriodReset = false;
			}
			if (FiscalPeriodSetupCreator.IsFixedLengthPeriod(year.FPType) == true)
			{
				DateTime? newPeriodStartDate = creator.CalcPeriodsStartDate(year.FirstFinYear);
				year.PeriodsStartDate = newPeriodStartDate;
			}
			else
			{
				RecalcPeriodStartDate(year);
			}

			try
			{
				this.skipPeriodReset = true;
				TYearSetup yr = (TYearSetup)this.FiscalYearSetup.Cache.Update(year);
			}
			finally
			{
				this.skipPeriodReset = false;
			}
			if (FiscalPeriodSetupCreator.IsFixedLengthPeriod(year.FPType) == false)
			{
				massUpdateFlag = true;
		...
		//protected abstract bool HasUnfinishedPeriods();
		...
		private bool AutoFillPeriods()
		{
			...
			//FiscalPeriodSetupCreator.FPType pType = setup.FPType;
			//setup.FinPeriods = FiscalPeriodSetupCreator<TPeriodSetup>.GetFiscalPeriodsNbr(pType, setup.HasAdjustmentPeriod.Value); //Correct Value for setup
			//setup.PeriodLength =(short?) FiscalPeriodSetupCreator.GetPeriodLength(pType);
			int count = 0;
		...
		private FiscalPeriodSetupCreator<TPeriodSetup> PeriodCreator
		{
			get
			{
				if (this._periodCreator == null)
				{
					...
					if (row != null)
					{
						if (row.FPType != FiscalPeriodSetupCreator.FPType.Custom)
						{
							this._periodCreator = new FiscalPeriodSetupCreator<TPeriodSetup>(row.FirstFinYear, row.PeriodsStartDate.Value, row.FPType, row.HasAdjustmentPeriod.Value, row.AdjustToPeriodStart.Value, row.PeriodLength, row.BegFinYear.Value, row.EndYearCalcMethod, row.EndYearDayOfWeek.Value);
						}
						else
						{
							short numberOfPeriods = (row.FinPeriods ?? 0);
							this._periodCreator = new FiscalPeriodSetupCreator<TPeriodSetup>(row.FirstFinYear, row.BegFinYear.Value, numberOfPeriods);
						}
					}
				}
	...
	public abstract class FiscalPeriodSetupCreator
	{
		...
		protected const string CS_YEAR_FORMAT = "0000";
		protected const string CS_PERIOD_NBR_FORMAT = "00";
