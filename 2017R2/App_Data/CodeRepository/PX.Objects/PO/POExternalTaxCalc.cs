...
namespace PX.Objects.PO
{
	public class POExternalTaxCalc : PXGraph<POExternalTaxCalc>
	{
		...
		public static void Process(List<POReceipt> list, bool isMassProcess)
		{
			POReceiptEntry rg = PXGraph.CreateInstance<POReceiptEntry>();
			for (int i = 0; i < list.Count; i++)
			{
				try
				{
					rg.Clear();
					rg.Document.Current = PXSelect<POReceipt, Where<POReceipt.receiptType, Equal<Required<POReceipt.receiptType>>, And<POReceipt.receiptNbr, Equal<Required<POReceipt.receiptNbr>>>>>.Select(rg, list[i].ReceiptType, list[i].ReceiptNbr);
					rg.CalculateAvalaraTax(rg.Document.Current);
					PXProcessing<POOrder>.SetInfo(i, ActionsMessages.RecordProcessed);
				}
				catch (Exception e)
