	...
	public class POCreate : PXGraph<POCreate>
	{
		...
		protected virtual IEnumerable fixedDemand()
		{
			foreach(PXResult<POFixedDemand,InventoryItem,Vendor,POVendorInventory> rec in 
				...
				And2<Where<SOOrder.orderNbr, Equal<Current<POCreateFilter.orderNbr>>, Or<Current<POCreateFilter.orderNbr>, IsNull>>,
				And2<Where<POFixedDemand.planDate, LessEqual<Current<POCreateFilter.requestedOnDate>>, Or<Current<POCreateFilter.requestedOnDate>, IsNull>>,
				And<Where<INItemClass.itemClassCD, Like<Current<POCreateFilter.itemClassCDWildcard>>, Or<Current<POCreateFilter.itemClassCDWildcard>, IsNull>>>>>>>>>>>.Select(this))
			{
				...
				{
					demand.EffPrice = APVendorPriceMaint.CalculateCuryUnitCost(FixedDemand.Cache, demand.VendorID, demand.VendorLocationID, demand.InventoryID, demand.SiteID, vendor.CuryID, demand.UOM, demand.OrderQty, (DateTime)Filter.Current.PurchDate, 0m);
			...
			if (row != null && Filter.Current != null)
			{
				...
				if (row != null && row.InventoryID != null && row.UOM != null && row.VendorID != null && Filter.Current.PurchDate != null)
				{
					...
					if (vendor != null && vendor.CuryID != null)
						vendorUnitCost = APVendorPriceMaint.CalculateCuryUnitCost(sender, row.VendorID, row.VendorLocationID, row.InventoryID, row.SiteID, vendor.CuryID, row.UOM, row.OrderQty, (DateTime)Filter.Current.PurchDate, 0m);
		...
		public static PXRedirectRequiredException CreatePOOrders(List<POFixedDemand> list, DateTime? PurchDate, bool extSort)
		{
			...
			list.Sort((a, b) =>
			{
				...
				if (a.PlanType == INPlanConstants.Plan90)
				{
					var ii = (InventoryItem)PXSelectorAttribute.Select<POFixedDemand.inventoryID>(docgraph.Caches[typeof(POFixedDemand)], a);
					aSortOrder = string.Format("ZZ.{0}", ii == null ? String.Empty : ii.InventoryCD);
				}
				else
				{
				SOLineSplit3 aline = PXSelect<SOLineSplit3, Where<SOLineSplit3.planID, Equal<Required<SOLineSplit3.planID>>>>.Select(docgraph, a.PlanID);
					...
					aSortOrder = aline == null ? String.Empty : string.Format("{0}.{1}.{2:D7}", aline.OrderType, aline.OrderNbr, aline.SortOrder.GetValueOrDefault());
				}

				if (b.PlanType == INPlanConstants.Plan90)
				{
					var ii = (InventoryItem)PXSelectorAttribute.Select<POFixedDemand.inventoryID>(docgraph.Caches[typeof(POFixedDemand)], b);
					bSortOrder = string.Format("ZZ.{0}", ii == null ? String.Empty : ii.InventoryCD);
				}
				else
				{
					...
					bSortOrder = bline == null ? String.Empty : string.Format("{0}.{1}.{2:D7}", bline.OrderType, bline.OrderNbr, bline.SortOrder.GetValueOrDefault());
				}
			...
			foreach (POFixedDemand demand in list)
			{
				...
				try
				{
				    ...
				    if (OrderType == POOrderType.RegularOrder && demand.PlanType != INPlanConstants.Plan6B)
					{
						line = setup.CopyLineDescrSO == true && soline != null
								? ordered.Find
									...
									POLine.requestedDate, POLine.tranDesc, POLine.projectID, POLine.taskID>
									(demand.VendorID, demand.VendorLocationID, demand.POSiteID, demand.InventoryID, demand.SubItemID,
									 ...
									 soline != null ? soline.TranDesc : null,
									 soline != null ? soline.ProjectID : null,
									 soline != null ? soline.TaskID : null)
								: ordered.Find
									<POLine.vendorID, POLine.vendorLocationID, POLine.siteID, POLine.inventoryID, POLine.subItemID,
									POLine.requestedDate, POLine.projectID, POLine.taskID>
									(demand.VendorID, demand.VendorLocationID, demand.POSiteID, demand.InventoryID, demand.SubItemID,
									 soline != null ? soline.ShipDate : null,
									 soline != null ? soline.ProjectID : null,
									 soline != null ? soline.TaskID : null);
					...
					if (soline != null)
					{
						...
						if (soline.VendorID != docgraph.Document.Current.VendorID)
						{
							soline.VendorID = docgraph.Document.Current.VendorID;

							SOLine5 origsoline = (SOLine5)docgraph.FixedDemandOrigSOLine.Select(soline.OrderType, soline.OrderNbr, soline.LineNbr);
							if (origsoline != null)
							{
								origsoline.VendorID = docgraph.Document.Current.VendorID;
								docgraph.FixedDemandOrigSOLine.Cache.SetStatus(origsoline, PXEntryStatus.Updated);
							}
						}
		...
		public partial class POCreateFilter : IBqlTable
		{
			...
			public abstract class requestedOnDate : PX.Data.IBqlField
			{
			}
