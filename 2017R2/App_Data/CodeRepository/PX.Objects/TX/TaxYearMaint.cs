...
namespace PX.Objects.TX
{
	public class TaxYearMaint: PXGraph<TaxYearMaint>
	{
		...
		public static TaxPeriod FindTaxPeriodByKey(PXGraph graph, int? branchID, int? taxAgencyID, string taxPeriodID)
		{
			return PXSelect<TaxPeriod,
								...
								.Select(graph, branchID, taxAgencyID, taxPeriodID);
		}

		public static TaxPeriod GetTaxPeriodByKey(PXGraph graph, int? branchID, int? taxAgencyID, string taxPeriodID)
		{
			var taxPeriod = FindTaxPeriodByKey(graph, branchID, taxAgencyID, taxPeriodID);
		...
		public static TaxPeriod FindFirstOpenTaxPeriod(PXGraph graph, int? branchID, int? taxAgencyID)
		{
			return PXSelect<TaxPeriod,
								Where<TaxPeriod.branchID, Equal<Required<TaxPeriod.branchID>>,
										And<TaxPeriod.vendorID, Equal<Required<TaxPeriod.vendorID>>,
		...
		public static TaxPeriod FindPreparedPeriod(PXGraph graph, int? branchID, int? taxAgencyID)
		{
			return PXSelect<TaxPeriod,
							Where<TaxPeriod.branchID, Equal<Required<TaxPeriod.branchID>>,
									And<TaxPeriod.vendorID, Equal<Required<TaxPeriod.vendorID>>,
									And<TaxPeriod.status, Equal<TaxPeriodStatus.prepared>>>>>
							.Select(graph, branchID, taxAgencyID);

		}

		public static TaxPeriod FindLastClosedPeriod(PXGraph graph, int? branchID, int? taxAgencyID)
		{
			return PXSelect<TaxPeriod,
							Where<TaxPeriod.branchID, Equal<Required<TaxPeriod.branchID>>,
									And<TaxPeriod.vendorID, Equal<Required<TaxPeriod.vendorID>>,
		...
		public static TaxPeriod FindLastTaxPeriod(PXGraph graph, int? branchID, int? taxAgencyID)
		{
			return PXSelect<TaxPeriod,
								Where<TaxPeriod.branchID, Equal<Required<TaxPeriod.branchID>>,
										And<TaxPeriod.vendorID, Equal<Required<TaxPeriod.vendorID>>>>,
		...
		public static TaxPeriod FinTaxPeriodByDate(PXGraph graph, int? branchID, int? taxAgencyID, DateTime? date)
		{
			return PXSelect<TaxPeriod,
					   Where<TaxPeriod.branchID, Equal<Required<TaxPeriodFilter.branchID>>,
							   ...
							   .Select(graph, branchID, taxAgencyID, date, date);
		}
		...
		public static TaxYear FindTaxYearByKey(PXGraph graph, int? branchID, int? taxAgencyID, string year)
		{
			return PXSelect<TaxYear,
							Where<TaxYear.branchID, Equal<Required<TaxYear.branchID>>,
									And<TaxYear.vendorID, Equal<Required<TaxYear.vendorID>>,
									And<TaxYear.year, Equal<Required<TaxYear.year>>>>>>
							.Select(graph, branchID, taxAgencyID, year);
		}
		...
		protected static TaxYearEx FindTaxYearExByKey(PXGraph graph, int? branchID, int? taxAgencyID, string year)
		{
			return PXSelect<TaxYearEx,
							Where<TaxYearEx.branchID, Equal<Required<TaxYearEx.branchID>>,
		...
		public class TaxYearFilter : PX.Data.IBqlTable
		{
			...
			/// </summary>
			[MasterBranch1099(Required = true, IsDBField = false)]
			...
			public abstract class shortTaxYear : PX.Data.IBqlField{}
			...
			protected class TaxYearSelector : PXCustomSelectorAttribute
			{
				public TaxYearSelector() : base(typeof(Search<TaxYear.year,
														Where<TaxYear.branchID, Equal<Current<TaxYearFilter.branchID>>,
				...
				protected virtual IEnumerable GetRecords()
				{
					...
					if (taxYearFilter.BranchID == null || taxYearFilter.VendorID == null)
						return new object[0];
					...
					if (taxYears.Any())
					{
						TaxPeriod lastPeriod = FindLastTaxPeriod(_Graph, taxYearFilter.BranchID, taxYearFilter.VendorID);
					...
					else
					{
						...
						TaxTran earliestTaxTran = null;

						using (new PXReadBranchRestrictedScope(taxYearFilter.BranchID))
						{
							earliestTaxTran = ReportTax.GetEarliestNotReportedTaxTran(_Graph, vendor);
						}
						...
						if (earliestTaxTran != null)
						{
							...
							TaxYearWithPeriods<TaxYear, TaxPeriod> taxYearWithPeriods = taxCalendar.Create(taxYearFilter.BranchID,
																											vendor,
																											tranDate.Value.Year);
						...
						{
							List<string> years = PXSelect<FinYear>.Select(_Graph)
																	.RowCast<FinYear>()
		...
		public class TaxYearEx : TaxYear
		{
			...
			public new abstract class year : PX.Data.IBqlField { }
		...
		public class TaxPeriodEx : TaxPeriod
		{
			...
			public new abstract class taxYear : IBqlField { }
		...
		public TaxYearMaint()
		{
			...
			RedirectToTaxDetailsReport.StateSelectingEvents += ReportRedirectionButtonFieldSelectingHandler;
			RedirectToTaxSummaryReport.StateSelectingEvents += ReportRedirectionButtonFieldSelectingHandler;
				...
				if (lastPersistablePeriod != null)
				{
					TaxYear yearAfterLastPersistable = PXSelect<TaxYear,
																	...
																	.SelectWindowed(this, 0, 1,
																					lastPersistablePeriod.BranchID,
					...
					if (yearAfterLastPersistable != null)
					{
						...
						new PXDataFieldRestrict(typeof(TaxYear.branchID).Name, PXDbType.Int, 4, taxYear.BranchID, PXComp.EQ),
						...
						new PXDataFieldRestrict(typeof(TaxPeriod.branchID).Name, PXDbType.Int, 4, taxYear.BranchID, PXComp.EQ),
		...
		protected virtual void ReportRedirectionButtonFieldSelectingHandler(PXCache sender, PXFieldSelectingEventArgs e)
		{
			e.ReturnState = PXButtonState.CreateDefaultState<TaxYearFilter>(e.ReturnState);

			if (TaxYearFilterSelectView.Current?.BranchID != null)
			{
				Branch branch = BranchMaint.FindBranchByID(this, TaxYearFilterSelectView.Current.BranchID);
				Ledger ledger = GeneralLedgerMaint.FindLedgerByID(this, branch.LedgerID);

				if (branch.BranchID == ledger.DefBranchID && ledger.PostInterCompany == true)
				{
		...
		protected virtual void DeleteButtonFieldSelectingHandler(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			TaxPeriod anyTaxPeriodInYear = PXSelect<TaxPeriod,
													Where<TaxPeriod.branchID, Equal<Current<TaxYearEx.branchID>>,
		...
		protected virtual void TaxYearFilter_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (!TaxYearFilterSelectView.Cache.ObjectsEqual<TaxYearFilter.branchID, TaxYearFilter.vendorID>(taxYearFilter, oldTaxYearFilter))
			{
				if (taxYearFilter.BranchID != null
				    && taxYearFilter.VendorID != null)
				{
					TaxPeriod deafaultTaxPeriod = TaxCalendar.GetOrCreateCurrentTaxPeriod(TaxYearSelectView.Cache,
																							TaxPeriodSelectView.Cache,
																							taxYearFilter.BranchID,
																							taxYearFilter.VendorID);

					taxYearFilter.Year = deafaultTaxPeriod?.TaxYear;

					TaxYearSelectView.Cache.Clear();
					TaxYearSelectView.Cache.ClearQueryCache();
					TaxPeriodSelectView.Cache.Clear();
					TaxPeriodSelectView.Cache.ClearQueryCache();
				}
				else
				{
				...
				&& taxYearFilter.Year != null)
			{
				...
				if (!TaxYearFilterSelectView.Cache.ObjectsEqual<TaxYearFilter.branchID, TaxYearFilter.vendorID, TaxYearFilter.year>(
					taxYearFilter, oldTaxYearFilter))
				{
					TaxYearExSelectView.Cache.Clear();
					TaxYearExSelectView.Cache.ClearQueryCache();
					TaxPeriodExSelectView.Cache.Clear();
					TaxPeriodExSelectView.Cache.ClearQueryCache();
		...
		protected virtual IEnumerable redirectToPrepareTaxReport(PXAdapter adapter)
		{
			    ...
			    && TaxYearFilterSelectView.Current.VendorID != null)
			{
				...
				taxPeriodFilter.BranchID = GetBranchIDForRedirection();
		...
		public virtual IEnumerable viewTaxPeriodDetails(PXAdapter adapter) 
		{
			...
			graph.History_Header.Current.BranchID = GetBranchIDForRedirection();
		...
		protected virtual int? GetBranchIDForRedirection()
		{
			Branch branch = BranchMaint.FindBranchByID(this, TaxYearFilterSelectView.Current.BranchID);
			Ledger ledger = GeneralLedgerMaint.FindLedgerByID(this, branch.LedgerID);

			return branch.BranchID == ledger.DefBranchID && ledger.PostInterCompany == true
				? null
				: TaxYearFilterSelectView.Current.BranchID;
		}
		...
		protected virtual void TryRedirectToReport(string reportPage)
		{
			...
			{
				int? branchID = GetBranchIDForRedirection();

				string branchCD = branchID != null
					? BranchMaint.FindBranchByID(this, TaxYearFilterSelectView.Current.BranchID).BranchCD
					: null;

				var reportParams = new Dictionary<string, string>
				{
					["MasterBranchID"] = branchCD,
		...
		protected virtual bool IsStartDateEditable(TaxYearFilter taxYearFilter)
		{
			...
			return anyTaxYearExcludingSelected == null && anyNotOpenedPeriod == null;
		}

		protected virtual bool IsTaxPeriodTypeEditable(TaxYearFilter taxYearFilter)
		{
			TaxYear lastTaxYear = FindLastTaxYear(this, taxYearFilter.BranchID, taxYearFilter.VendorID);
		...
		protected virtual TaxYearEx GetOrCreateTaxYearWithPeriodsInCacheByFilter(TaxYearFilter taxYearFilter)
		{
			...
			if (currentTaxYear != null)
			{
				...
				TaxPeriodEx anyExistingTaxPeriod = PXSelect<TaxPeriodEx,
																Where<TaxPeriodEx.branchID, Equal<Required<TaxPeriodEx.branchID>>,
				...
				{
					using (new PXReadBranchRestrictedScope(currentTaxYear.BranchID))
					{
												...
												PXSelectJoinGroupBy<TaxPeriodEx,
																LeftJoin<TaxReportLine,
																		On<TaxPeriodEx.vendorID, Equal<TaxReportLine.vendorID>,
																			And<TaxReportLine.netTax, Equal<True>>>,
																LeftJoin<TaxHistory,
																		On<TaxHistory.vendorID, Equal<TaxHistory.vendorID>,
																			And<TaxReportLine.lineNbr, Equal<TaxHistory.lineNbr>,
																			And<TaxPeriodEx.taxPeriodID, Equal<TaxHistory.taxPeriodID>>>>>>,
																Where<TaxPeriodEx.branchID, Equal<Current<TaxYearEx.branchID>>,
																...
																.SelectMultiBound(this, new object[] { currentTaxYear })
			...
			{
				TaxYearWithPeriods<TaxYearEx, TaxPeriodEx> taxYearWithPeriods = TaxCalendarEx.Create(taxYearFilter.BranchID,
