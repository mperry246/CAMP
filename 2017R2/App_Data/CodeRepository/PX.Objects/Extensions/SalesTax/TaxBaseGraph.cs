...
namespace PX.Objects.Extensions.SalesTax
{
         ...
         where TPrimary : class, IBqlTable, new()
    {
        ...
        {
            var res = SelectTaxes<Where<Tax.taxCalcLevel, Equal<CSTaxCalcLevel.calcOnItemAmt>,
        ...
        private void TaxSetLineDefault(object taxrow, Detail row)
        {
            ...
            decimal CuryTranAmt = row.CuryTranAmt ?? 0m;
            ...
            decimal InclRate = SumWithReverseAdjustment(incltaxes, typeof(TaxRev.taxRate));

            decimal CuryInclTaxAmt = SumWithReverseAdjustment(incltaxes, typeof(TaxDetail.curyTaxAmt));
            ...
            switch (tax.TaxCalcLevel)
            {
                ...
                case CSTaxCalcLevel.CalcOnItemAmtPlusTaxAmt:
                    ...
                    decimal CuryLevel1TaxAmt = SumWithReverseAdjustment(lvl1Taxes, typeof(TaxDetail.curyTaxAmt));
                    ...
                    || tax.TaxCalcLevel == CSTaxCalcLevel.CalcOnItemAmtPlusTaxAmt))
            {
                ...
                {
                    cache.SetValue<TaxDetail.curyOrigTaxableAmt>(taxdet, PXDBCurrencyAttribute.RoundCury(cache, taxdet, CuryTaxableAmt));
                }
            ...
            cache.SetValue<TaxDetail.curyTaxableAmt>(taxdet, PXDBCurrencyAttribute.RoundCury(cache, taxdet, CuryTaxableAmt));

            if (tax.DeductibleVAT == true)
            {
                cache.SetValue<TaxDetail.curyTaxAmt>(taxdet, PXDBCurrencyAttribute.RoundCury(cache, taxdet, CuryTaxAmt));
                taxdet.CuryExpenseAmt = PXDBCurrencyAttribute.RoundCury(cache, taxdet, CuryTaxAmt * (1 - (taxrev.NonDeductibleTaxRate ?? 0m) / 100));
                cache.SetValue<TaxDetail.curyTaxAmt>(taxdet, (decimal?)cache.GetValue<TaxDetail.curyTaxAmt>(taxdet) - taxdet.CuryExpenseAmt);
                decimal expenseAmt;
        ...
        protected virtual void CurrencyInfo_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
        {
            ...
            if (taxCalc == TaxCalc.Calc || taxCalc == TaxCalc.ManualLineCalc)
            {
                ...
                {
                    PXView siblings = CurrencyInfoAttribute.GetView(sender.Graph, GetDetailMapping().Table, GetDetailMapping().CuryInfoID);
        ...
        private TaxDetail TaxSummarizeOneLine(object row, SummType summType)
        {
            ...
            decimal? CuryTaxAmt = SumWithReverseAdjustment(
                taxitems,
                typeof(TaxDetail.curyTaxAmt));

            decimal? CuryExpenseAmt = SumWithReverseAdjustment(
                taxitems,
                typeof(TaxDetail.curyExpenseAmt));
        ...
        private decimal SumWithReverseAdjustment(List<Object> list, Type field)
        {
            decimal ret = 0.0m;
            list.ForEach(a =>
            {
                decimal? val = (decimal?)Base.Caches[BqlCommand.GetItemType(field)].GetValue(((PXResult)a)[BqlCommand.GetItemType(field)], field.Name);
        ...
        protected virtual void TaxTotal_RowInserting(PXCache cache, PXRowInsertingEventArgs e)
        {
            ...
            if (AvalaraMaint.IsExternalTax(cache.Graph, newdet.TaxZoneID) != true)
            {
