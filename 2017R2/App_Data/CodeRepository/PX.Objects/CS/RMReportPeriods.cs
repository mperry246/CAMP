...
namespace PX.Objects.CS
{
    ...
    {
        private List<FinPeriod> _finPeriods;
        ...
        public RMReportPeriods(PXGraph graph)
        {
            ...
            PXSelectOrderBy<FinPeriod, OrderBy<Asc<FinPeriod.startDate>>>.Select(graph);
            _finPeriods = new List<FinPeriod>(graph.Caches[typeof(FinPeriod)].Cached.Cast<FinPeriod>());
        }
        ...
        public string GetFinPeriod(string period, short? yearOffset, short? periodOffset)
        {
            ...
            if (!string.IsNullOrEmpty(period) && period.Length == 6)
            {
                ...
                if (yearOffset != null && yearOffset != 0)
                {
                    ...
                    List<FinPeriod> finPeriodsResYear = _finPeriods.Where(f => string.Compare(f.FinYear, resYear.ToString()) == 0).ToList();
                ...
                if (periodOffset != null && periodOffset != 0)
                {
                    ...
                    foreach (FinPeriod p in _finPeriods)
                    {
