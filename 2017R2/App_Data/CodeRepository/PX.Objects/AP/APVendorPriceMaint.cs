	...
	public class APVendorPriceMaint : PXGraph<APVendorPriceMaint>//, PXImportAttribute.IPXPrepareItems
	{
                                                     ...
                                                     bool alwaysFromBaseCurrency = false)
		{
			InventoryItem item = PXSelect<InventoryItem, 
                                 ...
                                 .Select(sender.Graph, inventoryID);
		...
		public virtual UnitCostItem FindUnitCostInt(PXCache sender, int? vendorID, int? vendorLocationID, int? inventoryID, int? siteID, string baseCuryID, string curyID, decimal? quantity, string UOM, DateTime date)
		{
			...
			PXSelectBase<APVendorPrice> unitCostBaseUOM = 
				...
				InnerJoin<InventoryItem, 
                      ...
                      And<InventoryItem.baseUnit, Equal<APVendorPrice.uOM>>>>,
				Where<APVendorPrice.inventoryID, Equal<Required<APVendorPrice.inventoryID>>,
				  And<APVendorPrice.vendorID, Equal<Required<APVendorPrice.vendorID>>,
				  And<APVendorPrice.curyID, Equal<Required<APVendorPrice.curyID>>,
			     And2<
			...
			APVendorPrice vendorPrice = unitCost.SelectWindowed(0, 1, inventoryID, vendorID, curyID, siteID, UOM, quantity, date, date, date, date);
            ...
            vendorPrice = unitCostBaseUOM.Select(inventoryID, vendorID, curyID, siteID, baseUnitQty, date, date, date, date);
        ...
        /// <returns/>      
        protected virtual UnitCostItem CreateUnitCostFromVendorPrice(APVendorPrice vendorPrice, string uom)
        {
            if (vendorPrice == null)
            {
                return null;
