...
namespace PX.Objects.AP
{
	...
	public class APAnyPeriodFilterableAttribute : AnyPeriodFilterableAttribute
	{
		public APAnyPeriodFilterableAttribute(Type aSearchType, Type aSourceType)
			: base(
				aSearchType, aSourceType, typeof(FinPeriod.finPeriodID), typeof(FinPeriod.descr), typeof(FinPeriod.active),
				typeof(FinPeriod.closed), typeof(FinPeriod.aPClosed))
		{
	...
	public class PaymentRefAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber, IPXFieldDefaultingSubscriber, IPXFieldVerifyingSubscriber 
	{
		...
		void IPXFieldVerifyingSubscriber.FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (pm != null && det != null 
				...
				&& IsNextNumberDuplicated(sender.Graph, det.CashAccountID, det.PaymentMethodID, number))
			{
        ...
        protected virtual void InsertCheck(PXCache sender, PaymentMethodAccount det, APRegister payment, string CheckNbr)
		{
			...
			foreach (string field in cache.Fields)
			{
				...
				if (NewValue != PXCache.NotSetValue)
				{
					...
					if (!string.IsNullOrEmpty(descr?.FieldName))
					{
		...
		void IPXRowPersistingSubscriber.RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (string.IsNullOrEmpty(newNumber))
			{
				...
				else if (string.IsNullOrEmpty(oldNumber))
				{
					if (det.APAutoNextNbr == true)
			{
						lastCheckNbr = newNumber;
					}
				}
				else if(check.IsMigratedRecord != true &&!string.Equals(oldNumber, newNumber))
				{
					if (det.APAutoNextNbr == true)
					{
					WebDialogResult result;
					if (check.IsReleaseCheckProcess == true)
					{
						result = WebDialogResult.Yes;
					}
					else
					{
						result = sender.Graph.Views[sender.Graph.PrimaryView].Ask(
							e.Row,
							Messages.AskConfirmation,
							string.Format(Messages.AskUpdateLastRefNbr, newNumber),
			...
			if (pm.PrintOrExport == true || pm.APRequirePaymentRef == true)
			{ 
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Update && !string.Equals(oldNumber, newNumber) ||
				...
				(e.Operation & PXDBOperation.Command) == PXDBOperation.Insert)
			{
				if (!string.IsNullOrEmpty(newNumber))
				{
					try
					{
						InsertCheck(sender, det, (APRegister) e.Row, newNumber);
					}
	...
	public class APTaxAttribute : ManualVATAttribute
	{
		...
		protected override List<object> SelectTaxes<WhereType>(PXGraph graph, object row, PXTaxCheck taxchk, params object[] parameters)
		{
			...
			{
				ParentCache(graph).GetValue(ParentRow(graph), _DocDate)
			};
	...
	public class APActiveProjectAttibute : PM.ProjectBaseAttribute
	{
		...
		public override void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			{
				if (project.BaseType == PM.PMProject.ProjectBaseType.Project)
				{
					if (project != null && project.NonProject != true && project.BaseType == PM.PMProject.ProjectBaseType.Project &&
	...
	public class APPaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjFinPeriodID, WhereSelect> :
		PaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjFinPeriodID,
			APPaymentChargeTran, APPaymentChargeTran.entryTypeID, APPaymentChargeTran.docType, APPaymentChargeTran.refNbr,
			WhereSelect>
	...
	public class PaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjFinPeriodID,
			ChargeTable, EntryTypeID, ChargeDocType, ChargeRefNbr, WhereSelect> : PXSelect<ChargeTable, WhereSelect>
		where PaymentTable : class, IBqlTable, new()
		where PaymentMethodID : IBqlField
		where CashAccountID : IBqlField
		where AdjDate : IBqlField
