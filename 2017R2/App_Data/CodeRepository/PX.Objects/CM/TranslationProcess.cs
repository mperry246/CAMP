...
namespace PX.Objects.CM
{
	...
	public class TranslationProcess : PXGraph<TranslationProcess>
	{
		...
		public partial class TranslationParams : PX.Data.IBqlTable
		{
			...
			#endregion
			#region DestLedgerId
			public abstract class destLedgerId : PX.Data.IBqlField
			{
			}
			...
			public abstract class lastFinPeriodID : PX.Data.IBqlField
			{
			}
		...
		public virtual IEnumerable translate(PXAdapter adapter)
		{
			...
			FinPeriod period = PXSelect<FinPeriod, Where<FinPeriod.finPeriodID, Equal<Required<TranslationParams.finPeriodID>>>>.Select(this, parameters.FinPeriodID);
			if (period.Active != true)
			{
				throw new PXException(Messages.FinPeriodIsNotActive);
			}
			...
			existHistory = (TranslationHistory)PXSelect<TranslationHistory,
													...
													OrderBy<Desc<TranslationHistory.finPeriodID>>>
							.Select(this, parameters.FinPeriodID, parameters.DestLedgerId);
											   ...
											   Select(TransHistGraph, parameters.TranslDefId))
			{
				...
				if (defdet.CalcMode == 1) //YTD Balance
				{
					...
					foreach (PXResult<GLHistoryByPeriod, Account, Sub, GLSetup, GLHistory> res in cmd.Select(
							...
							parameters.FinPeriodID,
							dcurr.TranslationGainAcctID,
							dcurr.TranslationLossAcctID,
							parameters.FinPeriodID.Substring(0, 4) + "01",
							AcctFromCD,
							AcctToCD,
							AcctFromCD,
							AcctToCD,
							...
							SubToCD))
					{
						...
						if (parameters.BranchID == null || parameters.BranchID == accthist.BranchID)
						{
							...
							histdet.FinPeriodID = parameters.FinPeriodID;
		...
		protected virtual void TranslationParams_CuryEffDate_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
		{
			TranslationParams row = (TranslationParams)e.Row;
			if (row == null) return;
			FinPeriod period = PXSelect<FinPeriod, Where<FinPeriod.finPeriodID, Equal<Current<TranslationParams.finPeriodID>>>>.Select(cache.Graph);
		...
		protected virtual void VerifyCurrencyEffectiveDate(PXCache sender, TranslationParams translationParameters)
			...
			FinPeriod currentPeriod = (FinPeriod)PXSelect<
				FinPeriod, 
				Where<
					FinPeriod.finPeriodID, Equal<Required<FinPeriod.finPeriodID>>>>
				.Select(sender.Graph, translationParameters.FinPeriodID);
		...
		{
			if (translationParameters?.FinPeriodID == null) return;
			...
			{
				if (currentPeriod.Active != true)
				{
					sender.RaiseExceptionHandling<TranslationParams.finPeriodID>(
						translationParameters,
						FinPeriodIDFormattingAttribute.FormatForError(translationParameters.FinPeriodID),
						new PXSetPropertyException(Messages.FuturePeriodIsNotAvalableForTranslation));
				}
				}
			else
			{
				sender.RaiseExceptionHandling<TranslationParams.finPeriodID>(
					translationParameters,
					FinPeriodIDFormattingAttribute.FormatForError(translationParameters.FinPeriodID),
					new PXSetPropertyException(GL.Messages.NoPeriodsDefined));
			}
		}
		...
		{
			TranslationParams translationParameters = e.Row as TranslationParams;

			if (translationParameters == null) return;

			VerifyFinancialPeriodID(sender, translationParameters);
			VerifyCurrencyEffectiveDate(sender, translationParameters);

			Translate.SetEnabled(!PXUIFieldAttribute.GetErrors(sender, translationParameters).Any());
		}
		#endregion
	...
	public class TranslationHistoryMaint : PXGraph<TranslationHistoryMaint>
	{
		...
		public virtual IEnumerable release(PXAdapter adapter)
		{
			...
			PXLongOperation.StartOperation(this, delegate() { CreateBatch(header, true); });
		...
		public static void CreateBatch(TranslationHistory thist, bool setError)
		{
		...
		public static void CreateBatchByHistDet(TranslationHistory thist, List<TranslationHistoryDetails> thistDetList, bool setError)
		{
			TranslationHistoryMaint grapth = PXGraph.CreateInstance<TranslationHistoryMaint>();
			grapth.Clear();
			if (setError == true)
			...
			batch.TranPeriodID = thist.FinPeriodID;
			if (thist.BranchID != null) batch.BranchID = thist.BranchID;
			...
			for (int i = 0; i < thistDetList.Count; i++)
			{
				...
				try
				{
					...
					tran.FinPeriodID = thistdet.FinPeriodID;
