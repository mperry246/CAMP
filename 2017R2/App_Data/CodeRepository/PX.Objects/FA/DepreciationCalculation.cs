...
namespace PX.Objects.FA
{
			...
			if (balance.BookID == null)
	{
		...
		{
			FABookPeriod deprToPeriod = FABookPeriodIDAttribute.FABookPeriodFromDate(graph, date, BookID);
			return deprToPeriod?.FinYear ?? date.Year.ToString("0000");
		}
		...
		protected virtual DeprCalcParameters SelectFromDatabase()
			...
			string deprFromYear = GetDBPeriodYearByDate(graph, DepreciationStartDate);
			...
			string deprToYear = GetDBPeriodYearByDate(graph, GetDatePlusYears(DepreciationStartDate, UsefulLife));
		...
		public string GetNextPeriodID(string periodID)
		{
			...
			int year = int.Parse(FiscalPeriodUtils.FiscalYear(periodID));
