...
namespace PX.Objects.FA
{
	...
	public class AssetGLTransactions : PXGraph<AssetGLTransactions>
	{
		...
		{
			public abstract class errorMessage: IBqlField {}
			[PXString]
		...
		protected virtual void FATran_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if(sender.ObjectsEqual<FATran.newAsset, 
				...
				sender.ObjectsEqual<FATran.receiptDate,
									...
									FATran.tranDate,
									FATran.finPeriodID,
									FATran.assetCD>(e.Row, e.OldRow)) return;
			...
			out int? assetID)
		{
			...
			FixedAsset asset = (FixedAsset)graph.Caches[typeof(FixedAsset)]
			...
			FALocationHistory location = (FALocationHistory)graph.Caches[typeof(FALocationHistory)]
			...
			FADetails det = (FADetails)graph.Caches[typeof(FADetails)]
			...
			foreach (FABookBalance bal in PXSelect<FABookSettings, 
				...
				.Where(bal => string.IsNullOrEmpty(location.PeriodID)))
			{
			...
			location.PeriodID = FinPeriodIDAttribute.FindFinPeriodByDate(graph, det.ReceiptDate)?.FinPeriodID;
			...
			foreach (FixedAsset asset in Caches[typeof(FixedAsset)].Inserted)
			{
				...
				if (assetdet != null)
				{
					...
					if (newhist != null)
					{
						...
						newhist.PeriodID = FinPeriodIDAttribute.FindFinPeriodByDate(this, assetdet.ReceiptDate)?.FinPeriodID;
					}
				}
			}
	...
	public partial class GLTranFilter : IBqlTable
	{
		...
		#endregion		
		#region ReconType
		public abstract class reconType : IBqlField
		{
			#region List
		...
		[FAOpenPeriod(typeof(GLTranFilter.tranDate))]
