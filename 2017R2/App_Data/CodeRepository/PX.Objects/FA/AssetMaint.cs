	...
	public class AssetMaint : PXGraph<AssetMaint, FixedAsset>
	{
		...
		public AssetMaint()
		{
			...
			foreach (FABook book in PXSelect<FABook>.Select(this))
			{
				...
				fname = $"{book.BookCode} Calculated";
				...
				fname = $"{book.BookCode} Depreciated";
				_FieldNames.Add(fname);
				AssetHistory.Cache.Fields.Add(fname);
		...
		public override void Persist()
		{
			...
			if (asset != null)
			{
					...
					&& register != null || !assetTranExists)
				{
					...
					{
						AssetBalance.Cache.SmartSetStatus(bookBalance, PXEntryStatus.Updated);
					}
				}
			...
			if (asset != null && det.ReceiptDate != null)
			{
				...
				if (glbal == null)
				{
					currentLocation.PeriodID = FinPeriodIDAttribute.FindFinPeriodByDate(this, det.ReceiptDate)?.FinPeriodID;
				...
				{
					currentLocation.PeriodID = trbal.TransferPeriodID;
				}
			...
			)
		{
			if (!adapter.MassProcess)
			{
				if (RevDispInfo.View.Answer == WebDialogResult.None)
				{
					...
					string PeriodID = FinPeriodIDAttribute.FindFinPeriodByDate(this, RevDispInfo.Current.ReverseDisposalDate)?.FinPeriodID;
		...
		protected virtual void FADetails_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			bool depreciated = IsDepresiated(tran);
		...
		{
			FADetails row = e.Row as FADetails;
			if (row == null) return;
			...
			else if (AssetProcess.IsFullyDepreciatedAsset(sender.Graph, row.AssetID))
			{
				status = FixedAssetStatus.FullyDepreciated;
			}

			sender.SetValue<FADetails.status>(row, status);
		}
		#endregion
			...
			foreach (object bal in new PXView(sender.Graph, false, cmd).SelectMulti())
			{
				...
				if ((state = (PXFieldState)cache.GetStateExt<Field>(bal)) != null && state.Enabled)
				{
					...
					try
					{
						...
						{
							((sender.Graph as DepreciationCalculation)?.Params ?? new DeprCalcParameters()).Fill(newBalance);
						}
						cache.Update(newBal);
					}
		...
		protected virtual void FABookBalance_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (!newitem)
			{
				FATran other = PXSelectReadonly<FATran, Where<FATran.assetID, Equal<Required<FABookBalance.assetID>>,
					...
					depreciated = IsDepresiated(other);
					bool purchased = depreciated || IsPurchased(other);
		...
		protected virtual void FABookBalance_DeprFromDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (details.ReceiptDate != null && e.NewValue != null &&
				details.ReceiptDate.Value.CompareTo(e.NewValue) > 0)
			{
		...
		protected virtual void FABookBalance_DeprFromPeriod_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			//
			foreach (PXResult<FABookPeriod, FABookHist> res in PXSelectJoin<FABookPeriod,
				LeftJoin<FABookHist, On<FABookHist.assetID, Equal<Required<FABookBalance.assetID>>,
				...
				OrderBy<Asc<FABookPeriod.finPeriodID>>>
				.Select(this, bal.AssetID, bal.BookID, fromPeriod, toPeriod))
			{
				...
				decimal? ptdDeprBase = isPeriodGreater
					...
					: string.CompareOrdinal(origHist.FinPeriodID, fromPeriod) == 0
						? origHist.YtdAcquired
		...
		private static bool IsDepresiated(FATran tran)
		{
			return tran != null && 
