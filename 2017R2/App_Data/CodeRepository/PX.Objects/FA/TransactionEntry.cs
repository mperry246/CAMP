...
namespace PX.Objects.FA
{
	...
	public class TransactionEntry : PXGraph<TransactionEntry, FARegister>
	{
		...
		protected virtual void FATran_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			FixedAsset asset = Asset.Current;
			if (reg == null || asset == null) return;

			if (reg.Origin == FARegister.origin.Adjustment)
			{
				if (asset.Depreciable == true)
					PXStringListAttribute.SetList<FATran.tranType>(Trans.Cache, null, new FATran.tranType.AdjustmentListAttribute().AllowedValues, new FATran.tranType.AdjustmentListAttribute().AllowedLabels);
				else
					PXStringListAttribute.SetList<FATran.tranType>(Trans.Cache, null, new FATran.tranType.NonDepreciableListAttribute().AllowedValues, new FATran.tranType.NonDepreciableListAttribute().AllowedLabels);
			}
		...
		protected virtual void FATran_FinPeriodID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			try
			{
				FABookPeriod period = PXSelect<FABookPeriod,
