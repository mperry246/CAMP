...
namespace PX.Objects.Common
{
	...
	/// </summary>
	internal static class RedirectionToOrigDoc
	{
        /// <summary>
		...
		public static void TryRedirect(string origDocType, string origRefNbr, string origModule)
		{
			if (string.IsNullOrWhiteSpace(origRefNbr) || string.IsNullOrWhiteSpace(origModule))
				return;

			PXGraph destinationGraph = PrepareDestinationGraph(origDocType, origRefNbr, origModule);
        ...
        private static PXGraph PrepareDestinationGraph(string origDocType, string origRefNbr, string origModule)
		{
            ...
            {
                case BatchModule.AP:
                    return PrepareDestinationGraphForAP(origDocType, origRefNbr);
				...
				case BatchModule.EP:
					return PrepareDestinationGraphForEP(origRefNbr);
		...
		private static PXGraph PrepareDestinationGraphForAP(string origDocType, string origRefNbr)
        {
            switch (origDocType)
            {
