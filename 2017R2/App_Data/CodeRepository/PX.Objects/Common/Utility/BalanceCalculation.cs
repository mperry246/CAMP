...
namespace PX.Objects.Common
{
	public static class BalanceCalculation
	{
		...
		/// of the adjusting document inside the <see cref="AdjustBalance"/> method.
		/// </summary>
		public static decimal GetDocumentBalanceSignCorrection(this IDocumentAdjustment application)
		{
			if (application.AdjgDocType == ARDocType.Refund
				|| application.AdjgDocType == ARDocType.VoidPayment && application.AdjdDocType == ARDocType.CreditMemo
				|| application.AdjgDocType == APDocType.Refund
				|| application.AdjgDocType == APDocType.VoidCheck && application.AdjdDocType == APDocType.DebitAdj)
			{
				return -1m;
			}
			...
			where TAdjustment : IAdjustmentAmount, IDocumentAdjustment
		{
			...
			{
				document.DocBal -= balanceAdjustment.BaseAdjustedBalanceDelta;
				document.CuryDocBal -= balanceAdjustment.CurrencyAdjustedBalanceDelta;
			}
			else if (application.IsOutgoingApplicationFor(document))
			{
				...
				document.DocBal -= balanceAdjustmentSign * balanceAdjustment.BaseAdjustingBalanceDelta;
				document.CuryDocBal -= balanceAdjustmentSign * balanceAdjustment.CurrencyAdjustingBalanceDelta;
			}
		}
