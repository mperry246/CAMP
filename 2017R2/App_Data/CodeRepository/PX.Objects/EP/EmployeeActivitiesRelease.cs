...
namespace PX.Objects.EP
{
	public class EmployeeActivitiesRelease : PXGraph<EmployeeActivitiesRelease>
	{
		...
		[CRCaseStatuses]
		[PXUIField(DisplayName = "Case Status")]
		protected virtual void CRCase_Status_CacheAttached(PXCache sender) {}
		...
		#region Selects
		[PXHidden]
		public PXSelect<CRCase> dummyCase;//CRCase_Status_CacheAttached
		[PXHidden]
		...
		public class EPActivityFilter : CR.OwnedFilter
		{
			...
			public abstract class tillDatePlusOne : IBqlField
			{
			}
			...
			[PXDimensionSelector(ContractAttribute.DimensionName, typeof(Search2<ContractExEx.contractID,
				...
				Where<ContractExEx.isTemplate, NotEqual<True>,
					And<ContractExEx.baseType, Equal<Contract.ContractBaseType>>>,
            ...
            for (int i = 0; i < activities.Count; i++)
            {
                ...
                try
                {
					if (activity.ProjectTaskID != null)//cost transactions are created only if project is set.
					{
						...
						decimal? cost = costEngine.CalculateEmployeeCost(activity, employee.BAccountID, activity.Date.Value);
						registerEntry.CreateTransaction(activity, employee.BAccountID, activity.Date.Value, activity.TimeSpent, activity.TimeBillable, cost);
