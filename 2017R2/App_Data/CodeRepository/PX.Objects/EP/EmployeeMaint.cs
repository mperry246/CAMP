...
namespace PX.Objects.EP
{
	...
	public class EmployeeMaint : PXGraph<EmployeeMaint>
	{
        ...
        protected void ApplyUserAccessRights(params PXSelectBase[] datamembers)
		{
			...
			{
				PXCache src = graph.Caches[view.Cache.GetItemType()];
		...
		protected virtual void EPEmployee_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			if (!sender.ObjectsEqual<EPEmployee.parentBAccountID>(e.Row, e.OldRow))
			{
				...
				if (!found)
				{
					...
					addr.BAccountID = ((EPEmployee)e.Row).ParentBAccountID ?? addr.BAccountID;
					addr.CountryID = company.Current.CountryID;
				...
				if (!found)
				{
					...
					cont.FullName = parent?.AcctName;
					cont.BAccountID = ((EPEmployee)e.Row).ParentBAccountID ?? cont.BAccountID;
					cont.DefAddressID = null;
					foreach (Address record in Address.Cache.Inserted)
					{
						cont.DefAddressID = record.AddressID;
		...
		protected virtual void EPEmployeeRate_EffectiveDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			DateTime? newVal = (DateTime?)e.NewValue;
			DateTime? nextEffectiveDate = GetNextEffectiveDate(row);
			DateTime? firstEffectiveDate = GetFirstEffectiveDate(row);
			if (newVal != null && nextEffectiveDate != null && newVal < nextEffectiveDate && newVal > firstEffectiveDate)
			{
				throw new PXSetPropertyException(Messages.DateMustBeGreaterOrLess, PXUIFieldAttribute.GetDisplayName<EPEmployeeRate.effectiveDate>(sender), nextEffectiveDate, firstEffectiveDate);
			}
		}
		...
		public virtual void EPEmployeeRate_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (row.RateType == RateTypesAttribute.Hourly)
			{
				...
				if (row.RegularHours == null || (decimal)(row.RegularHours) <= 0)
		...
		protected DateTime? GetFirstEffectiveDate(EPEmployeeRate currentRow)
		{
			DateTime? ret = null;
			PMTran lastTran =
				PXSelect<PMTran, Where<PMTran.resourceID, Equal<Current<EPEmployee.bAccountID>>>, OrderBy<Asc<PMTran.date>>>
					.SelectSingleBound(this, new object[] { Employee.Current }, null);
			EPEmployeeRate lastRate =
				PXSelect
					<EPEmployeeRate, Where<EPEmployeeRate.employeeID, Equal<Current<EPEmployee.bAccountID>>, And<EPEmployeeRate.rateID, NotEqual<Required<EPEmployeeRate.rateID>>>>,
						OrderBy<Asc<EPEmployeeRate.effectiveDate>>>.SelectSingleBound(this, new object[] { Employee.Current }, currentRow.RateID);
			if (lastRate != null && lastRate.EffectiveDate != null)
			{
				ret = lastRate.EffectiveDate.Value.AddDays(-1);
			}
        ...
        public override void Persist()
		{
            ...
            using (PXTransactionScope ts = new PXTransactionScope())
            {
                ...
                if (_employee != null && _employee.Status == Vendor.status.Inactive)
                {
                    ContactMaint graph = CreateInstance<ContactMaint>();
                    Contact contact = Contact.Current;
                    if (contact != null)
	...
	public class GenTimeCardFilter : IBqlTable
    {
        ...
        public abstract class lastDateGenerated : IBqlField { }
        ...
        public abstract class generateUntil : IBqlField { }
