    ...
    public class CompanyTreeMaint : PXGraph<CompanyTreeMaint>
    {
       ...
       )
        {
            ...
            Where<EPCompanyTreeMaster.parentWGID,
                ...
                OrderBy<Asc<EPCompanyTreeMaster.sortOrder>>>.Select(this, workGroupID))
            {
                if (!string.IsNullOrEmpty(item.Description) && item.WorkGroupID != SelectedFolders.Current.WorkGroupID)
                    yield return item;
            }
        }
        #endregion
        ...
        [PXUIField(DisplayName = "Add WorkGroup", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, Enabled = true)]
        [PXButton()]
        ...
        [PXUIField(DisplayName = "Delete WorkGroup", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, Enabled = true)]
        [PXButton()]
        ...
        [PXUIField(DisplayName = "Move WorkGroup", MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Select)]
        [PXButton()]
        ...
        private void deleteRecurring(EPCompanyTreeMaster map)
        {
            if (map != null)
            {
                foreach (EPCompanyTreeMaster child in PXSelect<EPCompanyTreeMaster,
                                                 Where<EPCompanyTreeMaster.parentWGID, Equal<Required<EPCompanyTreeMaster.workGroupID>>>>
