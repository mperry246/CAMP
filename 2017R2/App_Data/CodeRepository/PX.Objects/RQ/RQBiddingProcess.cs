...
namespace PX.Objects.RQ
{
	...
	{
		#region SingleMode
		public abstract class singleMode : PX.Data.IBqlField
		{
		}
	...
	public class RQBiddingProcess : PXGraph<RQBiddingProcess>
	{
		...
		public PXSelectJoin<RQBidding,
			...
			LeftJoin<Location, On<Location.bAccountID, Equal<RQBidding.vendorID>,
					 And<Location.locationID, Equal<RQBidding.vendorLocationID>>>>>,
			Where<RQBidding.reqNbr, Equal<Argument<string>>,
			And<RQBidding.lineNbr, Equal<Argument<int?>>>>> Bidding;
			...
			int? lineNbr)
		{
			...
			And<RQBidding.lineNbr, Equal<Required<RQRequisitionLine.lineNbr>>>>>.Select(this, State.Current.ReqNbr, State.Current.LineNbr))
			{
				RQBidding bidding = item;				
				bidding.Selected =
					bidding.OrderQty > 0 || (bidding.VendorID == this.Document.Current.VendorID && bidding.VendorLocationID == this.Document.Current.VendorLocationID);
				yield return item;
			}
		}
		...
		private void CopyUnitCost(RQRequisitionLine line, RQBidding bidding)
		{
			...
			foreach (RQRequisition rec in adapter.Get<RQRequisition>())
			{
				...
				if (item.Splittable == true)
				{
					if (item.VendorID == null || item.VendorLocationID == null)
					{
						...
						foreach (RQBidding b in PXSelect<RQBidding,
								Where<RQBidding.reqNbr, Equal<Required<RQBidding.reqNbr>>,
						...
						foreach (RQRequisitionLine line in this.Lines.Select(item.ReqNbr))
						{
							...
							{
								this.Lines.Cache.RaiseExceptionHandling<RQRequisitionLine.biddingQty>(
									line, line.BiddingQty,
							...
							if (biddings.TryGetValue(line.LineNbr, out linebidding))
							{
								...
								foreach (RQBidding b in linebidding)
								{
									if (b.OrderQty > 0)
									{
				...
				{
					Document.Cache.RaiseExceptionHandling<RQRequisition.vendorID>(item, null,
						new PXSetPropertyException(Messages.VendorIsNull));
					hasError = true;
				}
					...
					if (item.VendorID == null || item.VendorLocationID == null)
						foreach (RQRequisitionLine line in this.Lines.Select(item.ReqNbr))
						{
							if (line.OrderQty > line.BiddingQty)
							{
								...
								OrderBy<Asc<RQBidding.quoteUnitCost>>>.Select(this, line.ReqNbr, line.LineNbr))
								{
									Decimal quoteQty =
										bid.QuoteQty == 0 ?
										Decimal.MaxValue :
									...
									if (quoteQty > 0)
									{
										...
										if (qty >= bid.MinQty)
										{
											...
											{
												choosenVendor = new RQBiddingVendor() { VendorID = bid.VendorID, VendorLocationID = bid.VendorLocationID };
											}
		...
		protected virtual void RQRequisition_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible<RQRequisition.curyID>(sender, null,
		...
		protected virtual void RQBidding_Selected_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if(row.Selected == true && row.OrderQty.GetValueOrDefault() == 0)
			{
				...
				Where<RQRequisitionLine.reqNbr, Equal<Required<RQRequisitionLine.reqNbr>>,
					And<RQRequisitionLine.lineNbr, Equal<Required<RQRequisitionLine.lineNbr>>>>>.Select(this, row.ReqNbr, row.LineNbr);
		...
		protected virtual void RQBidding_CuryQuoteUnitCost_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				PXSelect<RQRequisitionLine, Where<RQRequisitionLine.reqNbr, Equal<Required<RQRequisitionLine.reqNbr>>,
					And<RQRequisitionLine.lineNbr, Equal<Required<RQRequisitionLine.reqNbr>>>>>.Select(this, row.ReqNbr ?? this.State.Current.ReqNbr, row.LineNbr ?? this.State.Current.LineNbr);

				Vendor vendor = PXSelect<Vendor, Where<Vendor.bAccountID, Equal<Required<Vendor.bAccountID>>>>.Select(this, row.VendorID);

				if (line != null && vendor != null && line.InventoryID.HasValue && row.CuryInfoID != null)
				{										
					POItemCostManager.ItemCost cost =
						POItemCostManager.Fetch(this,
						row.VendorID,	row.VendorLocationID, null, vendor.CuryID,  
						line.InventoryID, line.SubItemID, null, line.UOM);
					e.NewValue = cost.Convert<RQRequisitionLine.inventoryID, RQBidding.curyInfoID>(sender.Graph, line, row, line.UOM);
					e.Cancel = true;
				}
			}
		}
		...
		protected virtual void RQBidding_RowUpdating(PXCache sender, PXRowUpdatingEventArgs e)
		{
			...
			if (e.Cancel != true)
			{
				...
				if (row.VendorID != newRow.VendorID)
				{
					RQBiddingVendor vendor = ChoosenVendor.Select(row.ReqNbr, row.VendorID, row.VendorLocationID);
					if (vendor != null)
					{						
						row.CuryInfoID = vendor.CuryInfoID;
						sender.SetDefaultExt<RQBidding.curyQuoteUnitCost>(row);
					}
				}
			}
		}
		...
		protected virtual void RQBiddingVendor_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			RQBiddingVendor row = (RQBiddingVendor)e.Row;
			if (row == null) return;
			Vendor v = PXSelect<Vendor, Where<Vendor.bAccountID, Equal<Current<RQBiddingVendor.vendorID>>>>
		...
		{
			if (row.VendorLocationID != null)
				foreach (RQBidding sibling in Bidding.Select(
					row.ReqNbr ?? this.State.Current.ReqNbr, 
					row.LineNbr ?? this.State.Current.LineNbr))
				{
					if (sibling.VendorID == row.VendorID &&
					...
					{
						if (oldRow == null || oldRow.VendorID != row.VendorID)
							sender.RaiseExceptionHandling<RQBidding.vendorID>(
								row, row.VendorID, new PXSetPropertyException(ErrorMessages.DuplicateEntryAdded)
								);
						if (oldRow == null || oldRow.VendorLocationID != row.VendorLocationID)
							sender.RaiseExceptionHandling<RQBidding.vendorLocationID>(
								row, row.VendorLocationID, new PXSetPropertyException(ErrorMessages.DuplicateEntryAdded)
								);						
						return false;
					}
		...
		{
			if(row.VendorID == null || row.VendorLocationID == null) return;

			RQBiddingVendor bidder =
						PXSelect<RQBiddingVendor,
						Where<RQBiddingVendor.reqNbr, Equal<Required<RQBiddingVendor.reqNbr>>,
							And<RQBiddingVendor.vendorID, Equal<Required<RQBiddingVendor.vendorID>>,
							And<RQBiddingVendor.vendorLocationID, Equal<Required<RQBiddingVendor.vendorLocationID>>>>>>.SelectWindowed(this, 0, 1, row.ReqNbr, row.VendorID, row.VendorLocationID);
			if (bidder == null)
			{
