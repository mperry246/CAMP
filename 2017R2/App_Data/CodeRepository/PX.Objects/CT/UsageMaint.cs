...
namespace PX.Objects.CT
{
	public class UsageMaint : PXGraph<UsageMaint, UsageMaint.UsageFilter>
	{
		...
		public PXSelectJoin<
			PMTran, 
			LeftJoin<CRCase, 
				On<CRCase.noteID, Equal<PMTran.origRefID>, Or<CRCase.caseID, Equal<PMTran.caseID>>>>, 
		...
		public partial class UsageFilter : IBqlTable
		{
			...
			[PXDBInt]
			[PXSelector(typeof(Search<Contract.contractID, Where<Contract.isTemplate, Equal<boolFalse>, And<Contract.baseType, Equal<Contract.ContractBaseType>, And<Contract.status, NotEqual<Contract.status.draft>, And<Contract.status, NotEqual<Contract.status.inApproval>>>>>>), SubstituteKey = typeof(Contract.contractCD), DescriptionField = typeof(Contract.description))]
