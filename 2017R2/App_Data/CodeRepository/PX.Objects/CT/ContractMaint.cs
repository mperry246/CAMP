	...
	public class ContractMaint : PXGraph<ContractMaint, Contract>
	{
	   ...
	   [PXCopyPasteHiddenView]
			...
			if (contract.Type == Contract.type.Expiring || (contract.Type == Contract.type.Renewable && IsExpired(contract, renewalDate)))
		{
				...
				bool isAutonumbered = this.IsDimensionAutonumbered(ContractAttribute.DimensionName);
		...
		private void CreateExpiringRenewalHistory(Contract child)
		{
			if (child.OriginalContractID != null)
			{
				...
				RenewalHistory.Insert(new ContractRenewalHistory
				{
		...
		protected virtual void Contract_ContractCD_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			var selectContractTemplate = PXSelect<
				...
				And<ContractTemplate.isTemplate, Equal<True>>>>
					.Select(this, e.NewValue);
			var currentTemplate = selectContractTemplate.FirstOrDefault();
				...
				if (item != null)
				{
					if (item.IsBaseValid == true)
					{
						...
						if (inventory != null)
						{
							...
							DiscountEngine<ContractDetail>.SetLineDiscountOnly(sender, det,
								...
								ContractDetail.baseDiscountSeq, DiscountLineFields.manualDisc, DiscountLineFields.manualPrice, DiscountLineFields.lineType, DiscountLineFields.isFree>(sender, det), contract.DiscountID,
					...
					if (item.IsFixedRecurringValid == true || item.IsUsageValid == true)
					{
						...
						if (inventory != null)
						{
							DiscountEngine<ContractDetail>.SetLineDiscountOnly(sender, det,
								...
								ContractDetail.recurringDiscountSeq, DiscountLineFields.manualDisc, DiscountLineFields.manualPrice, DiscountLineFields.lineType, DiscountLineFields.isFree>(sender, det), contract.DiscountID,
					...
					if (item.IsRenewalValid == true)
					{
						...
						if (inventory != null)
						{
							DiscountEngine<ContractDetail>.SetLineDiscountOnly(sender, det,
								...
								ContractDetail.renewalDiscountSeq, DiscountLineFields.manualDisc, DiscountLineFields.manualPrice, DiscountLineFields.lineType, DiscountLineFields.isFree>(sender, det), contract.DiscountID,
		...
		public partial class ActivationSettingsFilter : IBqlTable
		{
			...
			#endregion
			#region ActionName
			public abstract class actionName : IBqlField
			{
			}
			[PXString]
