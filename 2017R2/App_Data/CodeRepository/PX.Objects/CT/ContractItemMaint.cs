...
namespace PX.Objects.CT
{
	public class ContractItemMaint : PXGraph<ContractItemMaint, ContractItem>
	{
		...
		[PXCopyPasteHiddenView]
		public PXSelectJoin<Contract, InnerJoin<ContractDetail, On<Contract.contractID, Equal<ContractDetail.contractID>>>, Where<ContractDetail.contractItemID, Equal<Current<ContractItem.contractItemID>>, And<Contract.isTemplate, Equal<False>, And<Contract.status, NotEqual<Contract.status.canceled>, And<Contract.status, NotEqual<Contract.status.expired>, And<Contract.status, NotEqual<Contract.status.draft>>>>>>> CurrentContracts;
		...
		private void SetControlsState(PXCache cache, ContractItem row)
		{
			ContractDetail activeDetail = PXSelectJoin<ContractDetail, InnerJoin<Contract, On<ContractDetail.contractID, Equal<Contract.contractID>>>,
					...
					And2<Where<Contract.status, Equal<Contract.status.active>, Or<Contract.status, Equal<Contract.status.inUpgrade>>>, And<Contract.isTemplate, Equal<False>>>>>.SelectWindowed(this, 0, 1, row.ContractItemID);
