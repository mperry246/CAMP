...
namespace PX.Objects.PM
{
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
		...
		public virtual List<BillingData> BillTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate, Dictionary<int, decimal> availableQty, Dictionary<int, PMDetail> billingItems)
		{
			...
			if (rule.FullDetail)
			{
				foreach(PMTran tran in billingBase )
				{
					...
					string subCD = CalculateTargetSalesSubaccountCD(rule, project, task, tran.ResourceID, line.BranchID, line.InventoryID, customer);
			...
			else
			{
				...
				foreach (Group group in grouping.BreakIntoGroups(billingBase))
				{
					...
					if (group.List.Count > 0)
					{
						...
						string subCD = CalculateTargetSalesSubaccountCD(rule, project, task, line.ResourceID, line.BranchID, line.InventoryID, customer);
		...
		public virtual string CalculateTargetSalesSubaccountCD(PMBillingRule rule, PMProject project, PMTask task, int? employeeID, int? branchID, int? inventoryID, Customer customer)
		{
			string result = null;
			...
			new object[] { rule.SubID, project.DefaultSubID, task.DefaultSubID, employeeSubID, inventorySubID, customerSubID, branchSubID },
			new Type[] { typeof(PMBillingRule.subID), typeof(PMProject.defaultSubID), typeof(PMTask.defaultSubID), typeof(EP.EPEmployee.salesSubID), typeof(InventoryItem.salesSubID), typeof(CR.Location.cSalesSubID), typeof(CR.LocationExtAddress.cMPSalesSubID) });
		...
		public virtual decimal? GetPrice(PMTran tran)
		{
			...
			if (tran.InventoryID != PMInventorySelectorAttribute.EmptyInventoryID)
			{
				...
				CM.CurrencyInfo dummy = GetCurrencyInfo(projectTask.CustomerID, billingDate);
				...
				if (dummy.CuryID != dummy.BaseCuryID)
				{
					decimal inBase;
					PXCurrencyAttribute.CuryConvBase(Caches[typeof(PMTran)], dummy, result.GetValueOrDefault(), out inBase);
					result = inBase;
				}
			}
		...
		public virtual CM.CurrencyInfo GetCurrencyInfo(int? customerID, DateTime? effectiveDate)
		{
			string curyID = CompanySetup.Current.BaseCuryID;
			string rateType = null;

			if (customerID != null)
			{
				Customer customer = PXSelect<Customer, Where<Customer.bAccountID, Equal<Required<Customer.bAccountID>>>>.Select(this, customerID);
				if (customer != null && !string.IsNullOrEmpty(customer.CuryID))
				{
					curyID = customer.CuryID;
					rateType = customer.CuryRateTypeID;
				}
			}
			...
			if (curyID == null || curyID == CompanySetup.Current.BaseCuryID)
			{
