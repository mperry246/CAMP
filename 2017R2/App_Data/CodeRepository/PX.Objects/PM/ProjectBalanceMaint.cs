...
namespace PX.Objects.PM
{
	...
	public class ProjectBalanceMaint : PXGraph<ProjectBalanceMaint>, PXImportAttribute.IPXPrepareItems
	{
		...
		[PXDefault]
		[Project(typeof(Where<PMProject.nonProject, Equal<False>, And<PMProject.isTemplate, Equal<False>>>), IsKey=true)]
		...
		public IEnumerable ViewProject(PXAdapter adapter)
		{
			if (Items.Current != null)
			{
				...
				graph.Project.Current = PXSelect<PMProject, Where<PMProject.contractID, Equal<Required<PMProject.contractID>>>>.Select(this, Items.Current.ProjectID);

				throw new PXPopupRedirectException(graph, Messages.ProjectEntry + " - " + Messages.ViewTask, true);
			}
			return adapter.Get();
		}
		...
		protected virtual void _(Events.RowSelected<PMBudget> e)
		{
			if (e.Row != null)
			{
				...
				PXUIFieldAttribute.SetEnabled<PMCostBudget.rate>(e.Cache, null, project?.BudgetFinalized != true);
				PXUIFieldAttribute.SetEnabled<PMCostBudget.qty>(e.Cache, null, project?.BudgetFinalized != true);
				PXUIFieldAttribute.SetEnabled<PMCostBudget.amount>(e.Cache, null, project?.BudgetFinalized != true);
			}
		}
