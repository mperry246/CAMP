...
namespace PX.Objects.AR
{
    ...
    public class ARAnyPeriodFilterableAttribute : AnyPeriodFilterableAttribute
	{
		...
		public ARAnyPeriodFilterableAttribute(Type aSearchType, Type aSourceType)
			: base(aSearchType, aSourceType, typeof(FinPeriod.finPeriodID),typeof(FinPeriod.descr), typeof(FinPeriod.active), typeof(FinPeriod.closed), typeof(FinPeriod.aRClosed))
		{
    ...
    public class CustomerCreditHelper<Entity, CustomerField, HoldField, ReleasedField> : PXSelectBase<Entity>
        where Entity : class, IBqlTable, new()
        where CustomerField : class, IBqlField
        where HoldField : class, IBqlField
        where ReleasedField : class, IBqlField
    {
		#region State
        ...
        #region Ctor
        public CustomerCreditHelper(PXGraph graph, Delegate @delegate)
		{
            _Graph = graph;
            View = new PXView(graph, false, new Select<Entity>(),@delegate);
            Initialize(graph);
        }

        public CustomerCreditHelper(PXGraph graph)
		{
            _Graph = graph;
            View = new PXView(graph, false, new Select<Entity>());
            Initialize(graph);
        }

        protected virtual void Initialize(PXGraph graph)
		{
            graph.RowSelected.AddHandler<Entity>(RowSelected);
            graph.RowPersisting.AddHandler<Entity>(RowPersisting);
            //it will place new handler after all the standard handlers
            graph.Initialized += OnGraphInitialize;
        }

        protected virtual void OnGraphInitialize(PXGraph graph)
					{
            graph.RowUpdated.AddHandler<Entity>(RowUpdated);
		}
        #endregion
        ...
        public virtual void RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			try
			{
				...
				if ((int?)sender.GetValue<CustomerField>(e.Row) != (int?)sender.GetValue<CustomerField>(e.OldRow))
				{
					sender.RaiseExceptionHandling<CustomerField>(e.Row, null, null);
					sender.RaiseExceptionHandling<HoldField>(e.Row, null, null);
				}
		...
		protected virtual void GetCustomerBalance(PXCache cache, Customer customer, out decimal? CustomerBal, out DateTime? OldInvoiceDate)
		{
			ARBalances summarybal;
			using (new PXConnectionScope())
			{
				IEnumerable<Override.ExtendedCustomer> sharedFamily =
					CustomerMaint.GetChildAccountsAndSelfStripped<Override.Customer.sharedCreditCustomerID>(cache.Graph, customer.SharedCreditCustomerID);
				...
				summarybal = CustomerMaint.GetCustomerBalances(cache.Graph, sharedFamilyIDs);
			}
		...
		{
			return (bool?)sender.GetValue<HoldField>(Row);
		}

		protected virtual bool? GetReleasedValue(PXCache sender, object Row)
		{
			return (bool?)sender.GetValue<ReleasedField>(Row);
		}
		...
		{
			PXCache arsetupcache = sender.Graph.Caches[typeof(ARSetup)];
			return ((ARSetup)arsetupcache.Current)?.MigrationMode;
		}

		protected virtual void PlaceOnHold(PXCache sender, object Row, bool OnAdminHold)
		{
			sender.RaiseExceptionHandling<HoldField>(Row, true, new PXSetPropertyException(Messages.CreditHoldEntry, PXErrorLevel.Warning));
		}
		...
		public virtual void Verify(PXCache sender, object Row, EventArgs e)
		{
			...
			int? CustomerID = (int?)sender.GetValue<CustomerField>(Row);
			...
			if (customer != null && customer.CreditRule != CreditRuleTypes.CS_NO_CHECKING && IsMigrationMode(sender) != true)
			{
				...
				if (customerclass != null && object.Equals(customerclass.CustomerClassID, customer.CustomerClassID) == false)
				{
...

				{
					...
					if (failed == false && (customer.CreditRule == CreditRuleTypes.CS_BOTH || customer.CreditRule == CreditRuleTypes.CS_DAYS_PAST_DUE))
					{
					...
					if (failed == false && (customer.CreditRule == CreditRuleTypes.CS_BOTH || customer.CreditRule == CreditRuleTypes.CS_CREDIT_LIMIT))
					{
					...
					{
						string existingError = PXUIFieldAttribute.GetError<CustomerField>(sender, Row);
						if (string.IsNullOrEmpty(existingError))
							sender.RaiseExceptionHandling<CustomerField>(Row, customer.AcctCD, new PXSetPropertyException(errmsg, PXErrorLevel.Warning));
					}
					...
					if (failed && HoldValue == false)
					{
						if (e is PXRowUpdatedEventArgs && (enforce || GetCreditCheckError(sender, Row) == true))
						{
							...
							sender.SetValueExt<HoldField>(Row, true);
							...
							if (DocumentBal > 0m)
							{
								...
								sender.SetValueExt<HoldField>(Row, false);
								UpdateARBalances(sender, Row, OldRow);
							}
							else
							{
                            ...
                            ((((PXRowPersistingEventArgs)e).Operation & PXDBOperation.Command) != PXDBOperation.Delete) && 
		...
		public virtual void UpdateARBalances(PXCache cache, object newRow, object oldRow)
		{
			if (oldRow != null)
			{
				ARReleaseProcess.UpdateARBalances(cache.Graph, (ARInvoice)oldRow, -((ARInvoice)oldRow).OrigDocAmt);
			}

			if (newRow != null)
			{
				ARReleaseProcess.UpdateARBalances(cache.Graph, (ARInvoice) newRow, ((ARInvoice) newRow).OrigDocAmt);
			}
		}
		#endregion
	}

	public class CustomerCreditHelperForInvoices : CustomerCreditHelper<ARInvoice, ARInvoice.customerID, ARInvoice.hold, ARInvoice.released>
	{
		public CustomerCreditHelperForInvoices(PXGraph graph, Delegate @delegate) : base(graph, @delegate) { }
		public CustomerCreditHelperForInvoices(PXGraph graph) : base(graph) { }
	}

	public class CustomerCreditHelperForPayments : CustomerCreditHelper<ARInvoice, ARInvoice.customerID, ARInvoice.hold, ARInvoice.released>
	{
		public CustomerCreditHelperForPayments(PXGraph graph, Delegate @delegate) : base(graph, @delegate) { }
		public CustomerCreditHelperForPayments(PXGraph graph) : base(graph) { }

		public override void UpdateARBalances(PXCache cache, Object newRow, Object oldRow)
		{
			if (oldRow != null)
			{
				ARReleaseProcess.UpdateARBalances(cache.Graph, (ARRegister)oldRow, -((ARRegister)oldRow).OrigDocAmt);
			}

			if (newRow != null)
			{
				ARReleaseProcess.UpdateARBalances(cache.Graph, (ARRegister)newRow, ((ARRegister)newRow).OrigDocAmt);
			}
		}
	}
	...
	public class ARTaxAttribute : TaxAttribute
	{
			...
			: base(ParentType, TaxType, TaxSumType)
		{
            ...
            Case<Where<ARTran.commissionable, Equal<True>, And<Where<ARTran.lineType, NotEqual<SO.SOLineType.discount>>>>,
                Mult<Mult<ARTran.curyTranAmt, ARTran.groupDiscountRate>, ARTran.documentDiscountRate>>>,
                decimal0>),
        ...
        protected override void ParentSetValue(PXGraph graph, string fieldname, object value)
        {
            ...
            if (fieldname == _CuryDocBal && cache.Current != null && ((ARSetup)cache.Current).InvoiceRounding != RoundingType.Currency)
            {
				...
				graph.Caches[typeof(CurrencyInfo)].ClearQueryCache();
        ...
        protected override void AdjustTaxableAmount(PXCache sender, object row, List<object> taxitems, ref decimal CuryTaxableAmt, string TaxCalcType)
        {
            ...
            decimal CuryDocDiscountTotal = (decimal)(ParentGetValue<ARInvoice.curyDocDisc>(sender.Graph) ?? 0m);
	...
	public class ARCashTranIDAttribute : CashTranIDAttribute
	{
		public ARCashTranIDAttribute(Type isMigrationModeEnabledSetupField)
			: base(isMigrationModeEnabledSetupField)
		{ }
	...
	public class AROpenPeriodAttribute : OpenPeriodAttribute
	{
		...
		public override void IsValidPeriod(PXCache sender, object row, object newValue)
		{
			...
			if (financialPeriod.ARClosed != true) return;
	...
	public class ARPaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjFinPeriodID, PMInstanceID, WhereSelect> :
		AP.PaymentChargeSelect<PaymentTable, PaymentMethodID, CashAccountID, AdjDate, AdjFinPeriodID,
			ARPaymentChargeTran, ARPaymentChargeTran.entryTypeID, ARPaymentChargeTran.docType, ARPaymentChargeTran.refNbr,
			WhereSelect>
