...
namespace PX.Objects.AR
{
	...
	public class ARSPCommissionProcess:  PXGraph<ARSPCommissionProcess>
	{
		...
		public partial class ARSalesPerTranExt: ARSalesPerTran 
		{
			...
			#endregion
			#region AveCommnPct
			public abstract class aveCommnPct : PX.Data.IBqlField
			{
			}
			[PXDecimal(6)]
			...
			#endregion
			#region CommnPct
			public new abstract class commnPct : PX.Data.IBqlField
			{
			}
			[PXDBDecimal(6)]
			...
			#endregion
			#region MinCommnPct
			public abstract class minCommnPct : PX.Data.IBqlField
			{
			}
			...
			#endregion
			#region CommnAmt
			public new abstract class commnAmt : PX.Data.IBqlField
			{
			}
			[PXDBBaseCury]
			...
			public new abstract class curyCommnAmt : PX.Data.IBqlField
			{
			}
			#endregion
			#region CommnblAmt
			public new abstract class commnblAmt : PX.Data.IBqlField
			{
			}
			...
			public new abstract class curyCommnblAmt : PX.Data.IBqlField
			{
			}
			#endregion
			#region ActuallyUsed
			public new abstract class actuallyUsed : PX.Data.IBqlField
			{
			}
			#endregion
			#region CommnPaymntPeriod
			public abstract new class commnPaymntPeriod : PX.Data.IBqlField
			{
			}
			#endregion
	...
	public static class SPCommissionCalendar
	{
			...
			where TPeriod : ARSPCommissionPeriod, new()
		{
			...
			else
			{
				...
				foreach (PXResult<FinYear, FinPeriod> res in PXSelectJoin<FinYear, 
																InnerJoin<FinPeriod, On<FinPeriod.finYear,
																Equal<FinYear.year>>>, 
																...
																OrderBy<Desc<FinYear.year>>>.Select(graph, (object)date))
				{
					if (new_year == null)
					{
						new_year = (ARSPCommissionYear)Year_Select.Select(((FinYear)res).Year);
						if (new_year == null)
						{
							new_year = CreateFrom((FinYear)res);
							new_year = (ARSPCommissionYear)Year_Select.Cache.Insert(new_year);
						}
					}
					else if (object.Equals(((FinYear)res).Year, new_year.Year) == false)
					{
						break;
					}
					ARSPCommissionPeriod new_per = CreateFrom((FinPeriod)res);
		...
		public static ARSPCommissionPeriod CreateFrom(FinPeriod aFiscalPeriod)
		{
		...
		public static ARSPCommissionYear CreateFrom(FinYear aFiscalYear)
		{
