...
namespace PX.Objects.AR
{
	...
	{
		void CreateWriteOff(ReasonCode reasonCode, string WOSubCD, DateTime? WODate, string WOFinPeriodID, ARRegister ardoc);
	...
	public class ARSmallBalanceWriteOffEntry : PXGraph<ARSmallBalanceWriteOffEntry>, IARWriteOffEntry
	{
		...
		protected virtual void ARAdjust_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			foreach (PXResult<ARInvoice, CurrencyInfo> res in PXSelectJoin<ARInvoice, InnerJoin<CurrencyInfo, On<CurrencyInfo.curyInfoID, Equal<ARInvoice.curyInfoID>>>, Where<ARInvoice.customerID, Equal<Current<ARPayment.customerID>>, And<ARInvoice.docType, Equal<Required<ARInvoice.docType>>, And<ARInvoice.refNbr, Equal<Required<ARInvoice.refNbr>>>>>>.Select(this, adjust.AdjdDocType, adjust.AdjdRefNbr))
			{
