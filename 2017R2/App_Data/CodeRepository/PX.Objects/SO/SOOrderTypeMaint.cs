...
namespace PX.Objects.SO
{
	...
	{
		public PXSelectJoin<SOOrderType, LeftJoin<SOOrderTypeOperation, On<SOOrderTypeOperation.orderType, Equal<SOOrderType.orderType>, And<SOOrderTypeOperation.operation, Equal<SOOrderType.defaultOperation>>>>,
		...
		protected new virtual IEnumerable Cancel(PXAdapter a)
		{
			...
			{
				PXResult<SOOrderType> orderType = PXSelectJoin<SOOrderType, LeftJoin<SOOrderTypeOperation, On<SOOrderTypeOperation.orderType, Equal<SOOrderType.orderType>, And<SOOrderTypeOperation.operation, Equal<SOOrderType.defaultOperation>>>>,
		...
		[PXDefault]
		[PXUIRequired(typeof(Where<Current<SOOrderType.active>, Equal<True>, And<Where<FeatureInstalled<FeaturesSet.vendorDiscounts>>>>))]
		...
		protected virtual void SOOrderType_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible<SOOrderType.recalculateDiscOnPartialShipment>(sender, e.Row, PXAccess.FeatureInstalled<FeaturesSet.inventory>() && PXAccess.FeatureInstalled<FeaturesSet.vendorDiscounts>());
		...
		protected virtual void SOOrderType_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
		{
			...
			SOLine soTran = PXSelect<SOLine, Where<SOLine.orderType, Equal<Required<SOOrderType.orderType>>>>.Select(this, ordertype.OrderType);
			if (soTran != null)
			{
