...
namespace PX.Objects.SO
{
	public class SOPackageEngine
	{
		...
		public virtual List<SOPackageInfoEx> PackByWeight(List<INItemBoxEx> boxes, ItemStats stats)
		{
			...
			List<BoxInfo> boxList = PackByWeight(boxes, stats.BaseWeight, stats.BaseQty);
		...
		public virtual List<BoxInfo> PackByWeight(List<INItemBoxEx> boxes, decimal baseWeight, decimal baseQty)
		{
			...
			if (baseWeight > 0 && boxes.Count > 0)
			{
				...
				else
				{
					...
					if (biggestBox.MaxNetWeight > 0)
					{
						...
						//remainder
						list.AddRange(PackByWeight(boxes, baseWeight, baseQty));
					}
				}
			}
		...
		public virtual List<INItemBoxEx> GetBoxesByInventoryID(int inventoryID, string carrierID)
		{
			...
			InventoryItem inventory = PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.SelectSingleBound(graph, null, inventoryID);
