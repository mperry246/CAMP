...
namespace PX.Objects.CA
{
	public class CATaxAttribute : ManualVATAttribute
	{
		...
		public CATaxAttribute(Type parentType, Type taxType, Type taxSumType, Type calcMode)
		...
		private int CalculateIndex<T>(List<object> ret, PXResult<Tax, TaxRev> line)
			where T : class, IBqlTable, new()
		{
			int idx;
			for (idx = ret.Count;
				(idx > 0) && string.Compare(((Tax)(PXResult<T, Tax, TaxRev>)ret[idx - 1]).TaxCalcLevel, ((Tax)line).TaxCalcLevel) > 0;
				idx--) ;
			return idx;
		}
