			...
			while (reader.Read())
			{
						...
						if (!String.IsNullOrEmpty(encoding))
				{
					...
					case XmlNodeType.ProcessingInstruction:
						...
						{
							var ValuesDict = reader.Value.Split(' ')
...
//		protected abstract void ReadOFXMessage(string aOFXMessage);
	...
	public class OFXStatementReader : OFXReaderBase, IStatementReader
	{
		...
		public void ExportTo(CABankStatementEntry graph, List<CABankStatement> aStatements)
		{
			bool updateCurrent = graph.casetup.Current.ImportToSingleAccount ?? false;
			foreach (STMTRS iAcctStatement in this.Content.Stmtrs)
			{
				Export(graph, iAcctStatement, updateCurrent);
		...
		protected static void Copy(CABankStatement aDest, STMTRS aSrc) 
		{
			...
			{
				aDest.StatementDate = aSrc.LEDGERBAL.DTASOF;
			}
