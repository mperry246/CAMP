...
namespace PX.Objects.CA
{
	...
	public class CAReleaseProcess : PXGraph<CAReleaseProcess>
	{
		...
		protected void CheckMultipleGLPosting(HashSet<long> caTranIDs)
		{
			foreach (long id in caTranIDs)
			{
				...
				if (count != 1)
				{
					...
					WriteDataIntegrityLog(errorMsg);
		...
		protected void WriteDataIntegrityLog(string errmsg)
		{
			DateTime now = DateTime.UtcNow;

			Common.DataIntegrity.DataIntegrityLog logEntry = this.Caches<Common.DataIntegrity.DataIntegrityLog>()
				.Insert(new Common.DataIntegrity.DataIntegrityLog
				{
					UtcTime = now,
					...
					ContextInfo = null,
					InconsistencyCode = new Common.DataIntegrity.DataIntegrityException().InconsistencyCode
				}) as Common.DataIntegrity.DataIntegrityLog;

			this.Caches<Common.DataIntegrity.DataIntegrityLog>().Persist(PXDBOperation.Insert);
			this.Caches<Common.DataIntegrity.DataIntegrityLog>().Persisted(false);
		}
	}
}
