...
namespace PX.Objects.CA.BankStatementProtoHelpers
{
	...
	[Serializable]
	public partial class CABankTranInvoiceMatch : IBqlTable
	{
		#region IsMatched
	...
	public static class StatementsMatchingProto
	{
		...
		{
			if (looseCompare)
				return graph.EvaluateMatching(aDetail.ExtRefNbr, aTran.ExtRefNbr, false, graph.matchSettings.Current.EmptyRefNbrMatching ?? true);
			else
				return graph.EvaluateTideMatching(aDetail.ExtRefNbr, aTran.ExtRefNbr, false, graph.matchSettings.Current.EmptyRefNbrMatching ?? true);
		}
                ...
                case GL.BatchModule.CA:
                    {
                        ...
                        {
                            CAAdj adj = PXSelect<CAAdj, Where<CAAdj.tranID, Equal<Required<CATran.tranID>>, And<CAAdj.cleared, NotEqual<True>>>>.Select(graph, aTran.TranID);
                            adj.Cleared = aTran.Cleared;
                            adj.ClearDate = aTran.ClearDate;
                            graph.Caches[typeof(CAAdj)].Update(adj);
                        }
                    }
                    break;
            }
