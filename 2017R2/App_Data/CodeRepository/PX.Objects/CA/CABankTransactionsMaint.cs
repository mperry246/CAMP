...
{
	public class CABankTransactionsMaint : PXGraph<CABankTransactionsMaint>
	{
		#region Constructor
		...
		public partial class Filter : IBqlTable
		{
			...
			[CashAccount(null, typeof(Search<CashAccount.cashAccountID, Where<Match<Current<AccessInfo.userName>>>>), IsKey = true)]
		...
		public PXSelect<CABankTran, Where<CABankTran.processed, Equal<False>,
			And<CABankTran.cashAccountID, Equal<Current<Filter.cashAccountID>>,
			And<CABankTran.tranType, Equal<Current<Filter.tranType>>,
			And<CABankTran.documentMatched, Equal<False>>>>>> UnMatchedDetails;
		...
		//these view are here for correct CABankStatementEntry.UpdateSourceDoc work
		...
		public PXSelect<CABankTranMatch, Where<CABankTranMatch.tranID, Equal<Required<CABankTran.tranID>>,
			...
			And<CABankTranMatch.docRefNbr, Equal<Required<CABankTranMatch.docRefNbr>>>>>>>> TranMatchInvoices;
		public PXSelectJoin<CABankTranAdjustment,
			...
			Where<CABankTranAdjustment.tranID, Equal<Optional<CABankTran.tranID>>>> Adjustments;
		...
		protected virtual IEnumerable detailMatchingInvoices()
		{
			...
			IEnumerable matches = this.FindDetailMatchingInvoices(detail, this.matchSettings.Current);
			if (matches.Any_())
			{
				...
				foreach (CABankTranInvoiceMatch it in matches)
				{
					...
					{
						bool matched = false;
						if (existingMatch != null && existingMatch.DocModule == invMatch.OrigModule
								&& existingMatch.DocType == invMatch.OrigTranType && existingMatch.DocRefNbr == invMatch.OrigRefNbr)
						{
							matched = true;
		...
		protected virtual IEnumerable details()
		{
			...
			Func<IEnumerable<CABankTran>> getTransactions = () =>
			{
				return PXSelect<CABankTran,
					...
					.Select(this, current.CashAccountID, current.TranType).RowCast<CABankTran>();
			};
		...
		protected virtual IEnumerable detailMatchesCA()
		{
			...
			cache.Clear();
			foreach (CATranExt CATran in this.FindDetailMatches(detail, this.matchSettings.Current, null))
			{
				CATranExt det = null;
		...
		public virtual IEnumerable UploadFile(PXAdapter adapter)
		{
			...
			if (doImport)
			{
				if (this.NewRevisionPanel.AskExt() == WebDialogResult.OK)
				{
					...
					this.Clear();
					Caches[typeof(CABankTran)].ClearQueryCache();
		...
		protected virtual void ClearMatchProc(CABankTran detail)
		{
			...
			{
				foreach (var match in TranMatch.Select(detail.TranID))
				{
		...
		{
			public abstract class ruleName : IBqlField { }
		...
		public virtual void ViewInvoice()
		{
			...
			switch (currentInvoice.OrigModule)
			{
				case BatchModule.AP:
					{
						document = (APInvoice)PXSelect<APInvoice, Where<APInvoice.docType, Equal<Required<APInvoice.docType>>, And<APInvoice.refNbr, Equal<Required<APInvoice.refNbr>>>>>
				...
				case BatchModule.AR:
					{
						document = (ARInvoice)PXSelect<ARInvoice, Where<ARInvoice.docType, Equal<Required<ARInvoice.docType>>, And<ARInvoice.refNbr, Equal<Required<ARInvoice.refNbr>>>>>
							.Select(this, currentInvoice.OrigTranType, currentInvoice.OrigRefNbr);
						if (document != null)
						{
		...
		{
			CABankTranInvoiceMatch row = e.Row as CABankTranInvoiceMatch;
			if ((Details.Current.DocumentMatched == true || Details.Current.CreateDocument == true) && (bool?)e.NewValue == true)
			{
		...
		protected virtual void CABankTranInvoiceMatch_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!sender.ObjectsEqual<CABankTranInvoiceMatch.isMatched>(row, oldRow))
			{
				...
				currentTran.DocumentMatched = row.IsMatched;
		...
		protected virtual void CABankTranAdjustment_AdjdCuryRate_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			CurrencyInfo pay_info = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Current<CABankTranAdjustment.adjgCuryInfoID>>>>.SelectSingleBound(this, new object[] { e.Row });
			CurrencyInfo vouch_info = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Current<CABankTranAdjustment.adjdCuryInfoID>>>>.SelectSingleBound(this, new object[] { e.Row });
		...
		protected virtual void CABankTranAdjustment_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (Details.Current != null && adj != null)
			{
				...
				Customer customer = PXSelect<Customer, Where<Customer.bAccountID, Equal<Required<Customer.bAccountID>>>>.Select(this, Details.Current.PayeeBAccountID);
		...
		protected virtual void CABankTranAdjustment_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if (row == null)
				return;
			CABankTran tranRow = PXSelect<CABankTran, Where<CABankTran.tranID, Equal<Required<CABankTranAdjustment.tranID>>>>.Select(this, row.TranID);
		...
		protected virtual void CABankTran_DocumentMatched_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				CABankTranMatch match = TranMatch.Select(row.TranID);
				if (match != null && !string.IsNullOrEmpty(match.DocRefNbr))
				{
			...
			if (row.OrigModule == GL.BatchModule.CA)
			{
				...
				CAEntryType entryType = PXSelectJoin<CAEntryType,
							...
							Where<CashAccountETDetail.accountID, Equal<Required<CABankTran.cashAccountID>>,
								...
								And<CAEntryType.entryTypeId, Equal<Required<CAEntryType.entryTypeId>>>>>>>.Select(this, row.CashAccountID, row.DrCr, entryTypeID);
		...
		protected virtual void CABankTran_EntryTypeId_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (row != null)
			{
				CAEntryType entryType = PXSelect<CAEntryType,
					Where<CAEntryType.entryTypeId, Equal<Required<CAEntryType.entryTypeId>>>>.Select(this, row.EntryTypeID);
				if (entryType != null)
				{
					...
					if (entryType.UseToReclassifyPayments == true && row.CashAccountID.HasValue)
					{
						CashAccount availableAccount = PXSelect<CashAccount, Where<CashAccount.cashAccountID, NotEqual<Required<CashAccount.cashAccountID>>,
						  And<CashAccount.curyID, Equal<Required<CashAccount.curyID>>>>>.SelectWindowed(sender.Graph, 0, 1, row.CashAccountID, row.CuryID);
		...
		protected virtual void CABankTran_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row.CreateDocument == false || row.OrigModule != GL.BatchModule.CA)
			{
				foreach (CABankTranDetail split in TranSplit.Select())
				{
					TranSplit.Delete(split);
			...
			if ((oldRow != null && oldRow.CreateDocument == true) && (row.CreateDocument == false || (row.OrigModule != GL.BatchModule.AR && row.OrigModule != GL.BatchModule.AP)))
			{
				foreach (CABankTranAdjustment adj in Adjustments.Select())
				{
					Adjustments.Delete(adj);
		...
		protected virtual void EnableTranFields(PXCache sender, CABankTran row)
		{
			...
			if (match != null)
			{
				matchedToCA = match.CATranID.HasValue;
				matchedToInv = !String.IsNullOrEmpty(match.DocRefNbr);
				if (matchedToCA && matchedToInv)
				{
					throw new PXException(Messages.ErrorInMatchTable, row.TranID);
				}
			}
			bool notMatched = !matchedToCA && !matchedToInv;
		...
		private void EnableCreateTab(PXCache sender, CABankTran row, bool needsPMInstance)
		{
			...
			bool noAdjustmentsYet = Adjustments.Select().Any() == false;
		...
		protected virtual void CABankTran_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row.MatchedToExisting == null)
			{
				...
				{
					CABankTranMatch match = ((CABankTranMatch)TranMatch.Select(row.TranID));
					row.MatchedToInvoice = CABankTranStatus.IsMatchedToInvoice(row, match);
		...
		protected virtual void CABankTranMatch_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			CABankTran currtran = PXSelect<CABankTran, Where<CABankTran.tranID, Equal<Required<CABankTran.tranID>>>>.Select(this, row.TranID);
		...
		protected virtual void CABankTranMatch_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			bool isAlreadyMatched = PXSelectReadonly2<CABankTranMatch, 
				InnerJoin<CABankTran, On<CABankTran.tranID, Equal<CABankTranMatch.tranID>>>, 
				Where<CABankTranMatch.cATranID, Equal<Required<CABankTranMatch.cATranID>>, 
				And<CABankTran.tranType, Equal<Current<CABankTran.tranType>>>>>.Select(this, row.CATranID)
				.Any(bankTran => Caches["CABankTranMatch"].GetStatus((CABankTranMatch)bankTran) != PXEntryStatus.Deleted);

			if (isAlreadyMatched)
			{
				CABankTran tranRow = PXSelect<CABankTran, Where<CABankTran.tranID, Equal<Required<CABankTranMatch.tranID>>>>.Select(this, row.TranID);
		...
		protected virtual void MatchSettings_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			MatchSettings settings = e.Row as MatchSettings;
			if (settings != null)
			{
				PXUIFieldAttribute.SetVisible<MatchSettings.matchInSelection>(sender, settings, false);
			}
		}
		...
		public static void DoProcessing(IEnumerable<CABankTran> list, Dictionary<int, PXSetPropertyException> listMessages)
		{
			...
			foreach (CABankTran iDet in list)
			{
				...
				graph.TranFilter.Current = new Filter() { CashAccountID = det.CashAccountID, TranType = det.TranType };
				try
				{
		...
		{
			CABankTranMatch firstmatch = TranMatch.SelectSingle(det.TranID);
			...
			try
			{
				...
				adj.AdjdDocType = firstmatch.DocType;
				adj.AdjdRefNbr = firstmatch.DocRefNbr;
				adj.AdjdModule = firstmatch.DocModule;

				Adjustments.Update(adj);
			}
			catch
			{
				throw new PXSetPropertyException(Messages.CouldNotAddApplication, firstmatch.DocRefNbr);
			}
		...
		protected virtual void MatchCATran(CABankTran det, List<Batch> externalPostList)
		{
			...
			if (match != null)
			{
				...
				{
					bool cleared = true;
					foreach (CATranExt tran in PXSelectJoin<CATranExt, InnerJoin<CABatchDetail,
						...
						Where<CABatchDetail.batchNbr, Equal<Required<CABatchDetail.batchNbr>>>>.Select(this, match.DocRefNbr))
					{
					...
					CABatch batch = PXSelectReadonly<CABatch, Where<CABatch.batchNbr, Equal<Required<CABatch.batchNbr>>>>.Select(this, match.DocRefNbr);
		...
		{
			Func<CATranExt> getCATran = () => (CATranExt)PXSelectReadonly<CATranExt, Where<CATranExt.tranID, Equal<Required<CATranExt.tranID>>>>.Select(this, tranID);
		...
		protected virtual void CreateDocumentProc(CABankTran aRow, bool doPersist)
		{
			...
			CurrencyInfo curyInfo = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>.Select(this, aRow.CuryInfoID);
			...
			if (aRow.OrigModule == GL.BatchModule.CA)
			{
				...
				foreach (PXResult<CABankTranDetail> res in TranSplit.Select(aRow.TranID, aRow.TranType))
				{
					...
					split.Qty = det.Qty;
					split.ReferenceID = det.ReferenceID;
		...
		{
			CABankStatementEntry.CheckCATransaction(parameters, CASetup.Current);
			return CABankStatementEntry.AddCATransaction(this, parameters, aCuryInfo, splits, IsTransferExpense);
		}
		...
		protected virtual PXResult<ARInvoice, ARAdjust> FindARInvoiceByInvoiceInfo(CABankTran aRow)
		{
			PXResult<ARInvoice, ARAdjust> invResult = (PXResult<ARInvoice, ARAdjust>)PXSelectJoin<ARInvoice,
										  ...
										  //										  And<ARInvoice.curyDocBal, Equal<Required<ARInvoice.curyDocBal>>,
										  And<ARInvoice.refNbr, Equal<Required<ARInvoice.refNbr>>>>>.Select(this, aRow.InvoiceInfo);
			...
			if (invResult == null)
			{
				invResult = (PXResult<ARInvoice, ARAdjust>)PXSelectJoin<ARInvoice,
								   ...
								   Where<ARInvoice.docType, Equal<AR.ARInvoiceType.invoice>,
									//									And<ARInvoice.customerID, Equal<Required<ARInvoice.customerID>>,
									//									And<ARInvoice.curyDocBal, Equal<Required<ARInvoice.curyDocBal>>,
									And<ARInvoice.invoiceNbr, Equal<Required<ARInvoice.invoiceNbr>>>>>.Select(this, aRow.InvoiceInfo);
		...
		protected virtual PXResult<APInvoice, APAdjust, APPayment> FindAPInvoiceByInvoiceInfo(CABankTran aRow)
		{
			...
			PXResult<APInvoice, APAdjust, APPayment> invResult = (PXResult<APInvoice, APAdjust, APPayment>)PXSelectJoin<APInvoice,
												...
												Where<APInvoice.docType, Equal<AP.APInvoiceType.invoice>,
													And<APInvoice.refNbr, Equal<Required<APInvoice.refNbr>>>>>.Select(this, aRow.InvoiceInfo);
								   ...
								   LeftJoin<APAdjust, On<APAdjust.adjdDocType, Equal<APInvoice.docType>,
										   ...
										   LeftJoin<APPayment, On<APPayment.docType, Equal<APInvoice.docType>,
											   ...
											   And<Where<APPayment.docType, Equal<APDocType.prepayment>,
												   Or<APPayment.docType, Equal<APDocType.debitAdj>>>>>>>>,
								   Where<APInvoice.docType, Equal<AP.APInvoiceType.invoice>,
									And<APInvoice.invoiceNbr, Equal<Required<APInvoice.invoiceNbr>>>>>.Select(this, aRow.InvoiceInfo);
        ...
        public virtual void DoMatch(PXCache aUpdateCache, IEnumerable<CABankTran> aRows, Dictionary<CABankTran, string> errorMatch)
		{
			...
			foreach (CABankTran det in aRows)
			{
				...
				{
					if (det.CountMatches.HasValue && det.CountMatches == 0 && det.CountInvoiceMatches.HasValue && det.CountInvoiceMatches == 0)
					{
		...
		protected virtual List<CABankTran> DoCAMatchInner(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			...
			foreach (object en in aRows)
			{
				...
				list = (List<CATranExt>)this.FindDetailMatches(iDet, this.matchSettings.Current, bestMatches);
		...
		public virtual void DoCAMatch(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			...
			foreach (CABankTran iRow in aRows)
			{
				...
				{
					CATranExt[] bestMatches = { null, null };
					this.FindDetailMatches(iRow, this.matchSettings.Current, bestMatches);
				}
			}
		}

		protected virtual List<CABankTran> DoInvMatchInner(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			Dictionary<string, List<CABankTranDocRef>> cross = new Dictionary<string, List<CABankTranDocRef>>();
			Dictionary<int, CABankTran> rows = new Dictionary<int, CABankTran>();
			IMatchSettings setting = this.matchSettings.Current;
			foreach (object en in aRows)
			{
				...
				List<CABankTranInvoiceMatch> list = null;
				CABankTranInvoiceMatch[] bestMatches = { null, null };
				list = (List<CABankTranInvoiceMatch>)this.FindDetailMatchingInvoices(iDet, this.matchSettings.Current, Decimal.Zero, bestMatches);
				if (bestMatches[0] != null
					&& (bestMatches[1] == null && bestMatches[0].MatchRelevance >= RelevanceTreshold
					|| bestMatches[1] != null && (bestMatches[0].MatchRelevance - bestMatches[1].MatchRelevance) > RelevanceTreshold
					|| bestMatches[0].MatchRelevance > InvoiceMatchTreshold))
				{
					CABankTranInvoiceMatch matchCandidate = bestMatches[0];
					CABankTranDocRef xref = new CABankTranDocRef();
					xref.Copy(iDet);
					xref.Copy(matchCandidate);
					xref.MatchRelevance = matchCandidate.MatchRelevance;
					string key = matchCandidate.OrigModule + matchCandidate.OrigTranType + matchCandidate.OrigRefNbr;
					if (cross.ContainsKey(key) == false)
					{
						cross.Add(key, new List<CABankTranDocRef>());
					}
		...
		public virtual void DoInvMatch(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			...
			while (spareDetails.Count > 0)
			{
				spareDetails = DoInvMatchInner(aUpdateCache, spareDetails);
			}
			...
			foreach (CABankTran iRow in aRows)
			{
				if (iRow.DocumentMatched == false && iRow.CreateDocument == false && iRow.CountInvoiceMatches > 0)
				{
					...
					this.FindDetailMatchingInvoices(iRow, this.matchSettings.Current);
				}
			}
		}
		...
		public virtual List<CABankTran> DoMatch(PXCache aUpdateCache, IEnumerable<CABankTran> aRows, Dictionary<string, List<CABankTranDocRef>> cross, Dictionary<int, CABankTran> rows)
		{
			...
			foreach (KeyValuePair<string, List<CABankTranDocRef>> iCandidate in cross)
			{
				...
				if (bestMatch != null && bestMatch.TranID != null)
				{
					...
					{
						detail = this.Details.SearchAll<Asc<CABankTran.tranID, Asc<CABankTran.tranID>>>(new object[] { bestMatch.TranID });
			...
			foreach (KeyValuePair<int, CABankTran> iDet in spareDetails)
			{
				...
				{
					detail = this.Details.SearchAll<Asc<CABankTran.tranID, Asc<CABankTran.tranID>>>(new object[] { iDet.Key });
					rows.Add(detail.TranID.Value, detail);
				}
				if (detail != null)
				...
				if (this.matchingTrans.TryGetValue(aDetail, out matchList))
				{
					...
					if (aDetail.MatchedToExisting == true && matchList.Find((CATranExt tran) => { return tran.IsMatched == true; }) == null)
					{
						matchList.ForEach((CATranExt tran) =>
							{
								tran.IsMatched =
								PXSelect<CABankTranMatch, Where<CABankTranMatch.tranID, Equal<Required<CABankTranMatch.tranID>>,
									And<CABankTranMatch.cATranID, Equal<Required<CABankTranMatch.cATranID>>>>>.Select(this, aDetail.TranID, tran.TranID).Count > 0
								||
								...
								PXSelect<CABankTranMatch, Where<CABankTranMatch.tranID, Equal<Required<CABankTranMatch.tranID>>,
									...
									And<CABankTranMatch.docModule, Equal<Required<CABankTranMatch.docModule>>,
									And<CABankTranMatch.docType, Equal<Required<CABankTranMatch.docType>>>>>>>.Select(this, aDetail.TranID, tran.OrigRefNbr, tran.OrigModule, tran.OrigTranType).Count > 0);
							});
					}
					return matchList;
				}
		...
		protected virtual IEnumerable FindDetailMatchingInvoices(CABankTran aDetail, MatchSettings aSettings)
		{
		...
		protected virtual IEnumerable FindDetailMatchingInvoices(CABankTran aDetail, MatchSettings aSettings, decimal aRelevanceTreshold, CABankTranInvoiceMatch[] aBestMatches)
		{
				...
				amount > 0)
			{
				PXSelectBase<Light.ARInvoice> sel = new PXSelectJoin<Light.ARInvoice,
					...
					Where<Light.ARAdjust.adjgRefNbr, IsNull,
						...
						And<Light.ARInvoice.curyID, Equal<Required<ARInvoice.curyID>>,
						And<Light.ARInvoice.curyDocBal, Equal<Required<ARInvoice.curyDocBal>>>>>>>>(this);
				if(aDetail.DrCr == CA.CADrCr.CADebit)
				{
					sel.WhereAnd<Where<Light.ARInvoice.docType, Equal<ARDocType.invoice>>>();
				}
				else
				{
					sel.WhereAnd<Where<Light.ARInvoice.docType, Equal<ARDocType.creditMemo>>>();
				}
				foreach (PXResult<Light.ARInvoice, Light.BAccount, CABankTranMatch, Light.ARAdjust, Light.CABankTranAdjustment> it in
					sel.Select(aDetail.TranType, aDetail.CuryID, amount, aDetail.TranDate))
				{
			...
			if (aDetail.DrCr == CA.CADrCr.CACredit && !clearingAccount)
			{
				foreach (PXResult<Light.APInvoice, Light.BAccount, CABankTranMatch, Light.APAdjust, Light.APPayment, Light.CABankTranAdjustment> it in
					PXSelectJoin<Light.APInvoice,
					InnerJoin<Light.BAccount, On<Light.BAccount.bAccountID, Equal<Light.APInvoice.vendorID>>,
					LeftJoin<CABankTranMatch, On<CABankTranMatch.docModule, Equal<GL.BatchModule.moduleAP>,
						...
						And<CABankTranMatch.tranType, Equal<Required<CABankTranMatch.tranType>>>>>>,
					LeftJoin<Light.APAdjust, On<Light.APAdjust.adjdDocType, Equal<Light.APInvoice.docType>,
						And<Light.APAdjust.adjdRefNbr, Equal<Light.APInvoice.refNbr>, And<Light.APAdjust.released, Equal<boolFalse>>>>,
					LeftJoin<Light.APPayment, On<Light.APPayment.docType, Equal<Light.APInvoice.docType>,
						And<Light.APPayment.refNbr, Equal<Light.APInvoice.refNbr>>>,
					LeftJoin<Light.CABankTranAdjustment, On<Light.CABankTranAdjustment.adjdDocType, Equal<Light.APInvoice.docType>,
					...
					Where<Light.APAdjust.adjgRefNbr, IsNull,
						...
						And<Light.APInvoice.curyDocBal, Equal<Required<APInvoice.curyDocBal>>,
						And<Light.APPayment.refNbr, IsNull,
							And<Where<Light.APInvoice.docDate, LessEqual<Required<Light.APInvoice.docDate>>,
							Or<Current<APSetup.earlyChecks>, Equal<True>>>>>>>>>>>>.Select(this, aDetail.TranType, aDetail.CuryID, amount, aDetail.TranDate))
				{
		...
		{
			return PXSelect<CABankTranAdjustment,
									Where<CABankTranAdjustment.adjdRefNbr, Equal<Required<CABankTranAdjustment.adjdRefNbr>>,
										...
										And<CABankTranAdjustment.adjdDocType, Equal<Required<CABankTranAdjustment.adjdDocType>>>>>>.Select(this, refNbr, module, docType).Count > 0;
		}
		protected virtual bool IsInvoiceMatched(string refNbr, string docType, string module)
		{
			return PXSelect<CABankTranMatch, Where<CABankTranMatch.docRefNbr, Equal<Required<CABankTranMatch.docRefNbr>>,
										And<CABankTranMatch.docModule, Equal<Required<CABankTranMatch.docModule>>,
										And<CABankTranMatch.docType, Equal<Required<CABankTranMatch.docType>>>>>>.Select(this, refNbr, module, docType).Count > 0;
		}
			...
			if (isMatched)
			{
				switch (cache.GetStatus(aTranMatch))
				{
					...
					default:
						...
						if (!cache.ObjectsEqual<CABankTranMatch.tranType,
			...
			if (!isMatched)
			{
				foreach (CABankTranMatch iMatch in cache.Inserted)
				{
					if (iMatch.TranType == aTran.TranType &&
						iMatch.DocModule == aMatch.OrigModule &&
						iMatch.DocType == aMatch.OrigTranType &&
						iMatch.DocRefNbr == aMatch.OrigRefNbr)
					{
		...
		public virtual decimal CompareRefNbr(CABankTran aDetail, CATran aTran, bool looseCompare)
		{
	...
	public class CABankIncomingPaymentsMaint : CABankTransactionsMaint
	{
		...
		[PXDefault(5, typeof(CASetup.receiptTranDaysBeforeIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_ReceiptTranDaysBefore_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(2, typeof(CASetup.receiptTranDaysAfterIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_ReceiptTranDaysAfter_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(5, typeof(CASetup.disbursementTranDaysBeforeIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_DisbursementTranDaysBefore_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(2, typeof(CASetup.disbursementTranDaysAfterIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_DisbursementTranDaysAfter_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(false, typeof(CASetup.allowMatchingCreditMemoIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_AllowMatchingCreditMemo_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(TypeCode.Decimal, "70.0", typeof(CASetup.refNbrCompareWeightIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_RefNbrCompareWeight_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(false, typeof(CASetup.emptyRefNbrMatchingIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_EmptyRefNbrMatching_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(TypeCode.Decimal, "20.0", typeof(CASetup.dateCompareWeightIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_DateCompareWeight_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(TypeCode.Decimal, "10.0", typeof(CASetup.payeeCompareWeightIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_PayeeCompareWeight_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(TypeCode.Decimal, "10.0", typeof(CASetup.dateMeanOffsetIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_DateMeanOffset_CacheAttached(PXCache sender) { }

		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXDefault(TypeCode.Decimal, "5.0", typeof(CASetup.dateSigmaIncPayments), PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void MatchSettings_DateSigma_CacheAttached(PXCache sender) { }
	}
}
