...
namespace PX.Objects.IN
{
	...
	public class INUnitAttribute : AcctSub2Attribute, IPXFieldVerifyingSubscriber, IPXRowSelectedSubscriber, IPXRowPersistingSubscriber
	{
		...
		public override Boolean DirtyRead
		{
		...
		public INUnitAttribute()
			: base()
		{
			Init(typeof(Search4<INUnit.fromUnit, Where<INUnit.unitType, Equal<INUnitType.global>>, Aggregate<GroupBy<INUnit.fromUnit>>>));
		}
			...
			: base()
		{
			...
			Type searchType = BqlTemplate.OfCommand<
				...
				And<INUnit.toUnit, Equal<Optional<BqlPlaceholder1>>>>>>
				.Replace<BqlPlaceholder1>(BaseUnitType).ToType();
			Init(searchType);
		}
			...
			: base()
		{
			...
			Type searchType = BqlTemplate.OfCommand<
				...
				Where<INUnit.unitType, Equal<INUnitType.inventoryItem>,
					And<INUnit.inventoryID, Equal<Optional<BqlPlaceholder1>>,
				Or<INUnit.unitType, Equal<INUnitType.global>,
					And<Optional<BqlPlaceholder1>, IsNull>>>>>>
				.Replace<BqlPlaceholder1>(InventoryType).ToType();
		...
		{
			_selectorNoAggregate = new PXSelectorAttribute(searchType);
			_selectorWithAggregate = new PXSelectorAttribute(BqlCommand.CreateInstance(searchType).AggregateNew<Aggregate<GroupBy<INUnit.fromUnit>>>().GetType());
			...
			where InventoryIDField : IBqlField
		{
			...
			if (sender.GetAttributes<InventoryIDField>().OfType<PXSelectorAttribute>().Count() > 0)
			{
				item = (InventoryItem)PXSelectorAttribute.Select(sender, Row, typeof(InventoryIDField).Name, InventoryID);
			}
			else
			{
	...
	public class SiteAvailAttribute : SiteAttribute, IPXFieldDefaultingSubscriber
	{
		...
		public SiteAvailAttribute(Type InventoryType)
		{
			...
			Type SearchType = BqlCommand.AppendJoin<
				LeftJoin<Address, On<Address.addressID, Equal<INSite.addressID>>,
			...
			_Select = BuildSelect(InventoryType);
		}
		...
		private static BqlCommand BuildSelect(Type InventoryType)
		{
			return
				BqlTemplate.OfCommand<
					Select2<InventoryItem,
					InnerJoin<INSite, On<INSite.siteID, Equal<InventoryItem.dfltSiteID>>>,
					Where<InventoryItem.inventoryID, Equal<Current<InventoryPh>>,
	...
	public class LocationAvailAttribute : LocationAttribute, IPXFieldDefaultingSubscriber
	{
			...
			: base(SiteIDType)
		{
			...
			Type search = BqlTemplate.OfCommand<
				...
				Where2<Match<Current<AccessInfo.userName>>, 
					And<INLocation.siteID, Equal<Optional<SiteIDPh>>>>>>
	...
	public class LocationAttribute : AcctSubAttribute
	{
			...
			: base()
		{
			...
			attr.CacheGlobal = false; //should ALWAYS be false, otherwise SiteID parameter will be ignored
			attr.DescriptionField = typeof(INLocation.descr);
			_Attributes.Add(attr);
			_SelAttrIndex = _Attributes.Count - 1;
		}
			...
			: this()
		{
			...
			if (_SiteIDType != null)
			{
				Type search = BqlTemplate.OfCommand<
						Search<INLocation.locationID,
						Where2<Match<Current<AccessInfo.userName>>,
						And<INLocation.siteID, Equal<Optional<BqlPlaceholder1>>>>>>
					.Replace<BqlPlaceholder1>(_SiteIDType)
					.ToType();
				
				var attr = new PXDimensionSelector.WithCachingByCompositeKeyAttribute(DimensionName, search, GetSiteIDKeyRelation(SiteIDType), typeof(INLocation.locationCD));
