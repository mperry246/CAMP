...
namespace PX.Objects.IN
{
	public class INReceiptEntry : PXGraph<INReceiptEntry, INRegister>, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void INRegister_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			sender.AllowDelete = (((INRegister)e.Row).Released == false && (((INRegister)e.Row).OrigModule == GL.BatchModule.IN || isPOTransfer));
            ...
            PXUIFieldAttribute.SetEnabled<INTran.branchID>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.inventoryID>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.subItemID>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.siteID>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.unitCost>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.tranCost>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.uOM>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.lotSerialNbr>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.expireDate>(transactions.Cache, null, sender.AllowUpdate && !isTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.projectID>(transactions.Cache, null, sender.AllowUpdate);
            PXUIFieldAttribute.SetEnabled<INTran.taskID>(transactions.Cache, null, sender.AllowUpdate);

            PXFieldState state;
            state = Caches[typeof(INTran)].GetStateExt(null, typeof(INTran.qty).Name) as PXFieldState;

            PXUIFieldAttribute.SetEnabled<INTran.reasonCode>(transactions.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled));
            PXUIFieldAttribute.SetEnabled<INTran.qty>(transactions.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled) && !isPOTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.lotSerialNbr>(transactions.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled) && !isPOTransfer);
            PXUIFieldAttribute.SetEnabled<INTran.subItemID>(transactions.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled) && !isPOTransfer);

            PXUIFieldAttribute.SetEnabled<INTranSplit.lotSerialNbr>(transactions.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled) && !isPOTransfer);
            PXUIFieldAttribute.SetEnabled<INTranSplit.subItemID>(transactions.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled) && !isPOTransfer);
            PXUIFieldAttribute.SetEnabled<INTranSplit.qty>(splits.Cache, null, sender.AllowUpdate && (isTransfer || state == null || state.Enabled) && !isPOTransfer);
        ...
        Where<INRegister.origModule, Equal<GL.BatchModule.moduleIN>, 
		...
		protected virtual void INTran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
            ...
            bool isTransfer = row.OrigRefNbr != null;

			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Update)
			{
				if (!string.IsNullOrEmpty(row.POReceiptNbr) && !isTransfer)
				{
