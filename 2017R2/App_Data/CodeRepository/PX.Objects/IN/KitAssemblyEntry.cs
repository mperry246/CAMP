...
namespace PX.Objects.IN
{
	public class KitAssemblyEntry : PXGraph<KitAssemblyEntry, INKitRegister>
	{
		...
		{
			return GetComponentSpecByID(inventoryID, null);
		}

		private INKitSpecStkDet GetComponentSpecByID(int? inventoryID, int? subItemID)
		{
	...
	public partial class INComponentTran : INTran
	{
		...
		public new abstract class tranCost : PX.Data.IBqlField { }
		#endregion
		#region ReasonCode
	...
	public partial class INKitRegister : IBqlTable, ILSPrimary
	{
		...
		protected String _TranPeriodID;
		[TranPeriodID(typeof(INKitRegister.tranDate), BqlField = typeof(INRegister.tranPeriodID))]
		public virtual String TranPeriodID
		{
			get
			{
		...
		public abstract class totalCostStock : PX.Data.IBqlField { }
		#endregion
		...
		public abstract class totalCostNonStock : PX.Data.IBqlField { }
		#endregion
		#region ControlQty
        ...
        [LocationAvail(typeof(INKitRegister.kitInventoryID),
					  ...
					  typeof(Where2<Where<INKitRegister.tranType, Equal<INTranType.assembly>, Or<INKitRegister.tranType, Equal<INTranType.disassembly>>>, And<INKitRegister.invtMult, Equal<shortMinus1>>>),
		...
		And<INComponentTranSplit.refNbr, Equal<Current<INKitRegister.refNbr>>>>>
	{
        ...
        public override void Availability_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
            if (!PXLongOperation.Exists(sender.Graph.UID))
            {
                IQtyAllocated availability = AvailabilityFetch(sender, (INComponentTran)e.Row, !(e.Row != null && ((INComponentTran)e.Row).Released == true));
		...
		protected override void RaiseQtyExceptionHandling(PXCache sender, object row, object newValue, PXSetPropertyException e)
		{
			...
			{
				sender.RaiseExceptionHandling<INComponentTran.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<INComponentTran.inventoryID>(row), sender.GetStateExt<INComponentTran.subItemID>(row), sender.GetStateExt<INComponentTran.siteID>(row), sender.GetStateExt<INComponentTran.locationID>(row), sender.GetValue<INComponentTran.lotSerialNbr>(row)));
			}
			else
			{
				sender.RaiseExceptionHandling<INComponentTranSplit.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<INComponentTranSplit.inventoryID>(row), sender.GetStateExt<INComponentTranSplit.subItemID>(row), sender.GetStateExt<INComponentTranSplit.siteID>(row), sender.GetStateExt<INComponentTranSplit.locationID>(row), sender.GetValue<INComponentTranSplit.lotSerialNbr>(row)));
			}
		}
	    ...
	    And<INKitTranSplit.refNbr, Equal<Current<INKitRegister.refNbr>>>>>
	{
		...
		public override void Availability_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			if (!PXLongOperation.Exists(sender.Graph.UID))
			{
				IQtyAllocated availability = AvailabilityFetch(sender, (INKitRegister)e.Row, !(e.Row != null && ((INKitRegister)e.Row).Released == true));
		...
		protected override void RaiseQtyExceptionHandling(PXCache sender, object row, object newValue, PXSetPropertyException e)
		{
			...
			{
				sender.RaiseExceptionHandling<INKitRegister.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<INKitRegister.kitInventoryID>(row), sender.GetStateExt<INKitRegister.subItemID>(row), sender.GetStateExt<INKitRegister.siteID>(row), sender.GetStateExt<INKitRegister.locationID>(row), sender.GetValue<INKitRegister.lotSerialNbr>(row)));
			}
			else
			{
				sender.RaiseExceptionHandling<INKitTranSplit.qty>(row, null, new PXSetPropertyException(e.MessageNoPrefix, PXErrorLevel.Warning, sender.GetStateExt<INKitTranSplit.inventoryID>(row), sender.GetStateExt<INKitTranSplit.subItemID>(row), sender.GetStateExt<INKitTranSplit.siteID>(row), sender.GetStateExt<INKitTranSplit.locationID>(row), sender.GetValue<INKitTranSplit.lotSerialNbr>(row)));
			}
		}
