...
namespace PX.Objects.IN
{
	public class INInventoryByItemClassEnq : PXGraph<INInventoryByItemClassEnq>
	{
		...
		public PXSelectJoin<InventoryItem, InnerJoin<INItemClass, On<InventoryItem.itemClassID, Equal<INItemClass.itemClassID>>>> Inventories;
		...
		private IEnumerable PerformPaste(PXAdapter adapter)
		{
			...
			foreach (InventoryItem inventory in inventories)
			{
				...
				var selectedinventory = Inventories.Locate(inventory);
				Inventories.SetValueExt<InventoryItem.selected>(selectedinventory, false);
			}
