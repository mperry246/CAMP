...
namespace PX.Objects.IN
{
	...
	{ 
		...
		public PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Optional<INKitSpecHdr.kitInventoryID>>>> KitItem;
		...
		protected virtual void INKitSpecHdr_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				InventoryItem item = KitItem.Select(row.KitInventoryID);
				if (item != null)
				{
		...
		protected virtual void INKitSpecStkDet_CompSubItemID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				if (res.Count > 0)
				{
					INSubItem subitem = PXSelect<INSubItem, Where<INSubItem.subItemID, Equal<Required<INSubItem.subItemID>>>>.Select(this, e.NewValue);
					var ex = new PXSetPropertyException(Messages.KitItemMustBeUniqueAccrosSubItems);
