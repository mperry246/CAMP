...
{
	using ComponentAmount = Tuple<InventoryItemComponentInfo, decimal>;
	...
	public class ScheduleCreator
	{
			...
			decimal? lineAmount)
		{
			...
			if (lineAmount != detailsTotal)
			{
				...
				{
					scheduleDetails.Single().TotalAmt = lineAmount;
					scheduleDetails.Single().DefAmt = lineAmount;
				}
				else if (scheduleDetails.HasAtLeastTwoItems())
				{
					...
					scheduleDetails.SkipLast(1).ForEach(scheduleDetail =>
					{
						...
						scheduleDetail.TotalAmt = corrected;
						scheduleDetail.DefAmt = corrected;
					});

					scheduleDetails.Last().TotalAmt = lineAmount - correctedTotal;
					scheduleDetails.Last().DefAmt = lineAmount - correctedTotal;
				}
			}
		}
