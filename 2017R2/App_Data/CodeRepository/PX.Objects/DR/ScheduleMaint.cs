...
namespace PX.Objects.DR
{
	...
	public class ScheduleMaint : PXGraph<ScheduleMaint>
	{
		...
		public PXSelect<DRScheduleDetail, Where<DRScheduleDetail.scheduleID, Equal<Current<DRScheduleDetail.scheduleID>>, And<DRScheduleDetail.componentID, Equal<Current<DRScheduleDetail.componentID>>>>> DocumentProperties;
		...
		protected virtual void DRScheduleDetail_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (correspondingSchedule != null)
			{
				...
				correspondingSchedule.FinPeriodID = scheduleDetail.FinPeriodID;
		...
		private void CreateTransactions(DRDeferredCode defCode, int? branchID)
		{
			if (Document.Current != null && Document.Current.Status == DRScheduleStatus.Draft)
			{
				...
				{
					IList<DRScheduleTran> tranList = GetTransactionsGenerator(defCode).GenerateTransactions(Schedule.Current, Document.Current, branchID);
		...
		public virtual void ReleaseCustomSchedule(DRSchedule schedule)
		{
			...
			{
				var details = PXSelect<DRScheduleDetail, Where<DRScheduleDetail.scheduleID, Equal<Required<DRSchedule.scheduleID>>, And<DRScheduleDetail.isResidual, Equal<False>>>>
		...
		private void CreateIncomingTransaction()
		{
			...
			if ( Document.Current.Module == BatchModule.AR )
				InitBalance(tran, Document.Current, sc, DeferredAccountType.Income);
			else
				InitBalance(tran, Document.Current, sc, DeferredAccountType.Expense);
		}
		...
		private void UpdateHistory()
		{
			...
			{
				if (Document.Current.Module == BatchModule.AR)
					UpdateBalanceProjection(tran, Document.Current, sc, DeferredAccountType.Income);
				else
					UpdateBalanceProjection(tran, Document.Current, sc, DeferredAccountType.Expense);
			}
		}
		
		public virtual void RebuildProjections(int? branchID)
		{
			...
			IList<DRScheduleTran> tranList = GetTransactionsGenerator(defCode).GenerateTransactions(sc, Document.Current, branchID);
			...
			if (existingTrans.Count > 0)
			{
				...
				DateTime lastPaymentPeriodStartDate = FinPeriodIDAttribute.PeriodStartDate(this, existingPeriods[existingPeriods.Count - 1]);

				List<DRScheduleTran> delete = new List<DRScheduleTran>();
				for (int i = 0; i < tranList.Count - 1; i++)
				{
					DateTime periodStartDate = FinPeriodIDAttribute.PeriodStartDate(this, tranList[i].FinPeriodID);
		...
		private void SubtractRevenueFromProjection(DRScheduleTran tran)
		{
			...
			hist.AcctID = Document.Current.AccountID;
			hist.SubID = Document.Current.SubID;
			hist.ComponentID = Document.Current.ComponentID ?? 0;
			hist.ProjectID = Document.Current.ProjectID ?? 0;
			hist.CustomerID = Document.Current.BAccountID ?? 0;
		...
		private void SubtractExpenseFromProjection(DRScheduleTran tran)
		{
			...
			hist.ComponentID = Document.Current.ComponentID ?? 0;
			hist.ProjectID = Document.Current.ProjectID ?? 0;
			hist.VendorID = Document.Current.BAccountID ?? 0;
		...
		private void SubtractRevenueFromBalance(DRScheduleTran tran)
		{
			...
			hist.FinPeriodID = tran.FinPeriodID;
			hist.AcctID = Document.Current.DefAcctID;
			hist.SubID = Document.Current.DefSubID;
			hist.ComponentID = Document.Current.ComponentID ?? 0;
			hist.ProjectID = Document.Current.ProjectID ?? 0;
			hist.CustomerID = Document.Current.BAccountID ?? 0;
