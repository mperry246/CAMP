...
namespace PX.Data
{
	...
	public class PXStringListAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXLocalizableList
	{
        ...
        /// <param name="isLocalizable">A Boolean value that indicates (if set to true) that the attribute labels</param>
			...
			if (!e.Cancel && e.NewValue != null)
			{
				...
				if (item == null)
				{
					...
					if (item != null)
					{
						...
						{
							CheckIntegrityAndPutGlobal(dict, @select.Cache, _SubstituteKey.Name, item, e.NewValue, false);
							CheckIntegrityAndPutGlobal(dict, @select.Cache, ForeignField.Name, item, ret, false);
						}
						e.NewValue = ret;
					}
					else
						...
						if (PXDatabase.IsReadDeletedSupported(_BqlType))
						{
							using (new PXReadDeletedScope())
							{
								...
								if (item != null)
								{
									...
									if (dict != null && select.Cache.GetStatus(item) == PXEntryStatus.Notchanged && select.Cache.Keys.Count <= 1)
									{
										CheckIntegrityAndPutGlobal(dict, @select.Cache, _SubstituteKey.Name, item, e.NewValue, true);
										CheckIntegrityAndPutGlobal(dict, @select.Cache, ForeignField.Name, item, ret, true);
									}
									e.NewValue = ret;
									found = true;
								}
	...
	public class PXRestrictorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber
	{
        ...
        public static object GetItem(PXCache cache, PXRestrictorAttribute attr, object data, object key)
        {
            ...
            if (row == null && (key == null || key.GetType() == cache.GetFieldType(attr._FieldName)))
            {
				...
				object item = PXResult.UnwrapMain(row = rows[0]);
