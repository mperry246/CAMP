...
namespace PX.Data
{
	...
	public class PXDBAttributeAttribute : PXDBFieldAttribute
	{
        ...
        /// <exclude/>
		public class AttributeIDPlaceholder : Constant<string>
		{
		...
		protected Type GetBqlTable(Type table)
		{
			while (typeof(IBqlTable).IsAssignableFrom(table.BaseType)
			       ...
			       && !table.IsDefined(typeof(PXTableNameAttribute), false))
			{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			Type st = _SingleSelect.GetType();
			Type ct = sender.GetItemType();
			while (ct != typeof(object) && st == _SingleSelect.GetType())
			{
				st = BqlCommand.Parametrize(ct, st);
				ct = ct.BaseType;
			}
			_View = new PXView(sender.Graph, true, BqlCommand.CreateInstance(st));
		}
