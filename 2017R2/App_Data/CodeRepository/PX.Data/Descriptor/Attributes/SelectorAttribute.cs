...
namespace PX.Data
{
	...
	public class PXSelectorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber, IPXFieldSelectingSubscriber, IPXDependsOnFields
	{
		...
		protected BqlCommand _PrimarySimpleSelect;
		...
		private static Type GetDBTableType(Type tableType)
		{
			...
			while (typeof(IBqlTable).IsAssignableFrom(dbTableType.BaseType)
					...
					&& !dbTableType.IsDefined(typeof(PXTableNameAttribute), false))
			{
		...
		internal sealed class GlobalDictionary : IPXCompanyDependent
		{
			...
			public static GlobalDictionary GetOrCreate(Type foreignTable, Type watchedTable, byte keysCount)
			{
				...
				if (dict == null)
					PXContext.SetSlot(foreignTable.FullName, dict = PXDatabase.GetSlot<GlobalDictionary>(slotName, watchedTable));
				return dict;
			}

			public static void ClearFor(Type table) => ClearFor(table, 1);
			public static void ClearFor(Type table, byte keysCount) => PXDatabase.ResetSlot<GlobalDictionary>(_GetSlotName(table, keysCount), table);
		...
		internal static object GetItem(PXCache cache, PXSelectorAttribute attr, object data, object key, bool unconditionally)
		{
			...
			{
				dict = attr.GetGlobalCache();
				lock (dict.SyncRoot) {
					GlobalDictionary.CacheValue cacheValue;
					if (dict.TryGetValue(attr.CreateGlobalCacheKey(cache, data, key), out cacheValue) && !cacheValue.IsDeleted && !(cacheValue.Item is IDictionary))
					{
						row = cacheValue.Item;
					}
				}
			...
			if (row == null && (key == null || key.GetType() == cache.GetFieldType(attr._FieldName)))
			{
				...
				if (attr._CacheGlobal && key != null && view.Cache.GetStatus(row) == PXEntryStatus.Notchanged && !PXDatabase.ReadDeleted && attr.CanCacheGlobal(view.Cache))
				{
					CheckIntegrityAndPutGlobal(dict, view.Cache, attr.ForeignField.Name, row, attr.CreateGlobalCacheKey(cache, data, key), false);
				}
			}
			return row;
		}
		...
		public static object GetField(PXCache cache, object data, string field, object value, string foreignField)
		{
			foreach (PXSelectorAttribute attr in cache.GetAttributesReadonly(data, field).OfType<PXSelectorAttribute>()) {
				...
				if (row == null) {
					...
					if (attr._CacheGlobal && value != null && !PXDatabase.ReadDeleted && attr.CanCacheGlobal(view.Cache))
					{
						CheckIntegrityAndPutGlobal(dict, view.Cache, attr.ForeignField.Name, row, attr.CreateGlobalCacheKey(cache, data, value), false);
					}
				}
		...
		internal static void CheckIntegrityAndPutGlobal(GlobalDictionary dict, PXCache foreignCache, String foreignField, Object foreignRow, Object ownKey, Boolean isRowDeleted, Boolean putSingle = false)
		{
				...
				|| String.Equals(foreignCache.Identity, foreignField, StringComparison.OrdinalIgnoreCase)))
			{
				lock (dict.SyncRoot)
				{
					dict.Set(compositeKey ?? ownKey, foreignRow, isRowDeleted);
					if (putBoth)
					{
						Object valKey = compositeKey == null ? val : Composite.Create(otherKeys.Append(val));
						dict.Set(valKey, foreignRow, isRowDeleted);
					}
				}
			}
		}
	    ...
	    {
			if (_CacheGlobal && dict != null && foreignItem != null && foreignCache.GetStatus(foreignItem) == PXEntryStatus.Notchanged && !PXDatabase.ReadDeleted && CanCacheGlobal(foreignCache))
			{
		...
		protected static string _GetSlotName(Type type, byte keysCount)
		{
			if (!PXDBLocalizableStringAttribute.IsEnabled)
			{
				return $"{keysCount}keys_" + type.FullName;
			}
			string language = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName ?? "";
			return $"{keysCount}keys_" + type.FullName + language.ToUpper();
		}
		/// <exclude/>
		public virtual void SelfRowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			if (sender.RowId == null || !CanCacheGlobal(sender) || sender._AggregateSelecting || sender._SingleTableSelecting) return;
				...
				if (item == null) {
					if (e.ReturnValue == null || e.ReturnValue.GetType() == sender.GetFieldType(_FieldName)) {
						...
						if (item != null) {
							if (_SubstituteKey != null) {
								...
								if (e.ReturnValue != null)
								{
									cacheOnReadItem(dict, itemCache, _SubstituteKey.Name, item, ret, deleted);
									cacheOnReadItem(dict, itemCache, ForeignField.Name, item, e.ReturnValue, deleted);
								}
								e.ReturnValue = ret;
							}
						}
					...
					if (e.ReturnValue == null)
					{
						...
						if (item != null) {
							...
							if (e.ReturnValue != null)
							{
								cacheOnReadItem(dict, itemCache, _SubstituteKey.Name, item, e.ReturnValue, deleted);
								cacheOnReadItem(dict, itemCache, ForeignField.Name, item, prevReturnValue, deleted);
							}
						}
					}
				}
			...
			if (!e.Cancel && e.NewValue != null) {
				...
				if (item == null) {
					...
					if (item != null) {
						...
						cacheOnReadItem(dict, @select.Cache, _SubstituteKey.Name, item, e.NewValue, false);
						cacheOnReadItem(dict, @select.Cache, ForeignField.Name, item, ret, false);
						e.NewValue = ret;
					}
					else {
						using (new PXReadBranchRestrictedScope()) {
		...
		public virtual void SubstituteKeyCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
        {
            ...
            if (ShouldPrepareCommandForSubstituteKey(e)) {
				...
				foreach (PXDBFieldAttribute attr in sender.GetAttributes(_FieldName).OfType<PXDBFieldAttribute>())
	            {
		            ...
		            e.FieldName = BqlCommand.SubSelect + e.SqlDialect.quoteTableAndColumn(extTable, _SubstituteKey.Name) + " FROM " +
								BqlCommand.GetTableName(_Type, sender.Graph) + " " + extTable + " "
								+ "WHERE " + e.SqlDialect.quoteTableAndColumn(extTable, ForeignField.Name) + " = "
								+ e.SqlDialect.quoteTableAndColumn(e.Table?.Name ?? (isInnerSubselect ? attr.BqlTable.Name : _BqlTable.Name),
																	sender.BqlSelect == null ? attr.DatabaseFieldName : _FieldName)
								+ ")";
		            if (e.Value != null)
		            {
			            e.DataValue = e.Value;
		...
		public virtual void ReadDeletedFieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (item != null) {
				if (view.Cache.GetStatus(item) == PXEntryStatus.Notchanged) {
					if (dict != null && !PXDatabase.ReadDeleted && CanCacheGlobal(view.Cache)) {
						CheckIntegrityAndPutGlobal(dict, view.Cache, ForeignField.Name, item, CreateGlobalCacheKey(sender, e.Row, key), deleted);
					}
					if (deleted) {
						throw new PXForeignRecordDeletedException();
					}
