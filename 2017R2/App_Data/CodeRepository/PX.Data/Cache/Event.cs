...
namespace PX.Data
{
    ...
    /// <remarks>
    ///   <para>The <tt>CommandPreparing</tt> event is triggered each time the <span><span>Acumatica</span></span> Data Access Layer prepares a database-specific SQL
    ...
    /// <tt>PXCommandPreparingEventArgs</tt>.<tt>FieldDescription</tt> object that is used in the generation of an SQL <span>statemen</span>t.</para>
    ///   <para>The <tt>CommandPreparing</tt> event handler is used to:</para>
    ///   <list type="bullet">
    ///     <item>Exclude a <span>DAC</span> field from a SELECT, INSERT, or UPDATE operationDAC field from DACELECT, INSERT, or UPDATE operation</item>
    ...
    /// </remarks>
    /// <example>
    ///   <code title="Example" description="According to the naming convention for the graph event handlers in Acumatica Framework, the event handler must have the following signature." lang="CS">
    ...
    ///   ... 
    /// }</code>
    ///   <code title="Example2" description="The code below excludes a DAC field from the UPDATE operation." lang="CS">
    ...
    ///     }
    /// }</code>
    ///   <code title="Example3" description="The code below replaces a DAC field with a custom T-SQL statement." groupname="Example" lang="CS">
		...
		public sealed class FieldDescription : ICloneable, IEquatable<FieldDescription>
	{
			...
			internal FieldDescription(Type bqlTable, string fieldName, PXDbType dataType, int? dataLength, object dataValue, bool isRestriction, bool isExcludedFromUpdate=false)
			{
				BqlTable = bqlTable;
				FieldName = fieldName;
				DataType = dataType;
			...
			public object Clone()
			{
				return new FieldDescription(BqlTable, FieldName, DataType, DataLength, DataValue, IsRestriction, IsExcludedFromUpdate);
		}
        ...
        {
            return new FieldDescription(_BqlTable, _FieldName, _DataType, _DataLength, _DataValue, _IsRestriction, _isExcludedFromUpdate);
        }
    ...
    ///   <para>The <tt>RowSelecting</tt> event handler is used to:</para>
    ///   <list type="bullet">
    ///     <item>Calculate DAC field values that are not bound to specific database columns.</item>
    ///     <item>Convert the database table value of a DAC field to its presentation form.</item>
    ///   </list>
    ///   <innovasys:widget type="Note Box" layout="block" xmlns:innovasys="http://www.innovasys.com/widgets">
    ///     <innovasys:widgetproperty layout="block" name="Content">The application developer can execute additional BQL statements within a RowSelecting event
    ///     handler. However, the connection scope used to retrieve data, which triggered the RowSelecting event, is still busy at the moment, so no other operations
    ///     on this connection scope are allowed. Therefore, to execute additional BQL statements in a RowSelecting handler, it is necessary to use a separate
    ///     connection scope.</innovasys:widgetproperty>
    ///   </innovasys:widget>
    ...
    ///     ...
    /// }</code>
    ///   <code title="Example4" description="The code below converts the database table value of a DAC field to the internal presentation." groupname="Example2" lang="CS">
    /// public class PXDBCryptStringAttribute : PXDBStringAttribute, 
    ///                                         IPXFieldVerifyingSubscriber, 
    ///                                         IPXRowUpdatingSubscriber, 
    ///                                         IPXRowSelectingSubscriber
    ...
    ///     ...
    /// }</code>
    ///   <code title="Example4" description="The code below validates a data record while it is being committed to the database." groupname="Example2" lang="CS">
